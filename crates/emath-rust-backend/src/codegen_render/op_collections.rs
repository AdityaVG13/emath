...
fn authored_list_concat(
    ...
    let target = super::numeric_element_kind(values, kinds);
    let ValueKind::Vector(element) = &target else {
        return Err(BackendError::UnsupportedType(
            "list concatenation needs an element carrier".into(),
        ));
    };
    let target_ty = crate::rust_ir::render::render_ty(&target.rust_ty()?);
    let mut parameters = Vec::new();
    let mut arguments = Vec::new();
    let mut lengths = String::new();
    let mut extend = String::new();
    for (index, value) in values.iter().enumerate() {
        let source = kind_at(kinds, *value);
        let ValueKind::Vector(source_element) = &source else {
            return Err(BackendError::UnsupportedType(
                "list concatenation requires sequences".into(),
            ));
        };
        let source_ty = source_element.rust_ty().or_else(|_| element.rust_ty())?;
        let name = format!("__list_part_{index}");
        parameters.push(format!(
            "{name}: &[{}]",
            crate::rust_ir::render::render_ty(&source_ty)
        ));
        ...
        lengths.push_str(&format!("count = count.checked_add({name}.len()).ok_or_else(|| String::from(\"concatenated length exceeds usize\"))?; "));
        let item = Expr::Raw(
            if source_element.is_copy() {
                "*__list_element"
            } else {
                "__list_element"
            }
        ...
        if let Some(converted) = super::numeric_boundary_value(&item, source_element, element) {
            extend.push_str(&format!(
                "for __list_element in {name} {{ output.push({}); }} ",
                render_expr(&converted)
            ));
        } else {
            extend.push_str(&format!("output.extend_from_slice({name}); "));
        }
    }
    // Borrow each input once outside the closure's binding scope. Counting
    // lengths must not clone vectors or evaluate an inlined operand twice.
    Ok(map_runtime_result(format!(
        "(|{}| -> Result<{target_ty}, String> {{ let mut count = 0usize; {lengths}let mut output = Vec::new(); output.try_reserve_exact(count).map_err(|_| String::from(\"list allocation exceeds available capacity\"))?; {extend}Ok(output) }})({})",
        parameters.join(", "),
        arguments.join(", ")
    )))
}
    ...
    match op {
        EmirOp::ToF64(value) => {
            if !matches!(kind_at(kinds, *value), ValueKind::F64 | ValueKind::I64) {
                return Err(BackendError::UnsupportedType(
                    "to-f64 requires a numeric scalar".into(),
                ));
            }
            Ok(Expr::Raw(format!(
                "({}) as f64",
        ...
        EmirOp::DenseLayout(value) => {
            ...
            Ok(Expr::Raw(match kind_at(kinds, *value) {
                ...
                ValueKind::Vector(element) if *element == ValueKind::F64 => {
                    format!("emath_rt::DenseLayout::Vector(({code}).len())")
                }
                ValueKind::Matrix(element) if *element == ValueKind::F64 => format!(
                    "emath_rt::DenseLayout::Matrix {{ rows: ({code}).rows(), cols: ({code}).cols(), len: ({code}).as_slice().len() }}"
                ),
                ValueKind::Tensor => format!(
                    "emath_rt::DenseLayout::Tensor {{ shape: ({code}).shape.clone(), len: ({code}).data.len() }}"
                ),
                _ => {
                    return Err(BackendError::UnsupportedType(
                        "dense-layout requires a numeric carrier".into(),
                    ));
                }
            }))
        }
        EmirOp::VectorSlice {
            ...
            if kind_at(kinds, *vector) != ValueKind::Vector(Box::new(ValueKind::F64))
                || kind_at(kinds, *offset) != ValueKind::I64
                || kind_at(kinds, *count) != ValueKind::I64
            {
                return Err(BackendError::UnsupportedType(
                    "vector-slice requires Float64 storage and Int bounds".into(),
                ));
            }
        ...
        EmirOp::VectorConcat(values) => {
            ...
            let values = values
                ...
                .collect::<Vec<_>>();
            let mut code =
                String::from("(|| -> Result<Vec<f64>, String> { let mut count = 0usize; ");
            for value in &values {
                code.push_str(&format!("count = count.checked_add(({value}).len()).ok_or_else(|| String::from(\"concatenated length exceeds usize\"))?; "));
            }
            ...
            for value in &values {
                code.push_str(&format!("output.extend_from_slice(&{value}); "));
            }
            code.push_str("Ok(output) })()");
            Ok(map_runtime_result(code))
        }
        EmirOp::ListConcat(values) => authored_list_concat(values, program, kinds),
        EmirOp::SameDenseShape(left, right) => {
            ...
            let expression = match (kind_at(kinds, *left), kind_at(kinds, *right)) {
                ...
                (ValueKind::Vector(a_kind), ValueKind::Vector(b_kind))
                    if *a_kind == ValueKind::F64 && *b_kind == ValueKind::F64 =>
                {
                    format!("({a}).len() == ({b}).len()")
                }
                (ValueKind::Matrix(a_kind), ValueKind::Matrix(b_kind))
                    if *a_kind == ValueKind::F64 && *b_kind == ValueKind::F64 =>
                {
                    format!("({a}).rows() == ({b}).rows() && ({a}).cols() == ({b}).cols()")
                }
                (ValueKind::Tensor, ValueKind::Tensor) => {
                    format!("({a}).shape == ({b}).shape && ({a}).data.len() == ({b}).data.len()")
                }
                _ => "false".into(),
            };
        ...
        EmirOp::DenseValues(value) => {
            ...
            Ok(match kind_at(kinds, *value) {
                ...
                ValueKind::Vector(element) if *element == ValueKind::F64 => {
                    owned_operand(program, *value, kinds)
                }
                ValueKind::Matrix(element) if *element == ValueKind::F64 => {
                    Expr::Raw(format!("({code}).as_slice().to_vec()"))
                }
                ValueKind::Tensor => Expr::Raw(format!("({code}).data.clone()")),
                _ => {
                    return Err(BackendError::UnsupportedType(
                        "dense-values requires a numeric carrier".into(),
                    ));
                }
            })
        }
        EmirOp::DenseRepack { template, data } => {
            if kind_at(kinds, *data) != ValueKind::Vector(Box::new(ValueKind::F64)) {
                return Err(BackendError::UnsupportedType(
                    "dense-repack requires Float64 data".into(),
                ));
            }
            ...
            let ValueKind::DenseLayout(kind) = kind_at(kinds, *template) else {
                return Err(BackendError::UnsupportedType(
                    "dense-repack requires a layout".into(),
                ));
            };
            let (pattern, result) = match *kind {
                ValueKind::F64 | ValueKind::I64 => ("emath_rt::DenseLayout::Scalar", "data[0]"),
                ValueKind::Vector(element) if *element == ValueKind::F64 => {
                    ("emath_rt::DenseLayout::Vector(_)", "data")
                }
                ValueKind::Matrix(element) if *element == ValueKind::F64 => (
                    "emath_rt::DenseLayout::Matrix { rows, cols, .. }",
                    "emath_rt::Matrix::new(*rows, *cols, data).map_err(str::to_owned)?",
                ),
                ValueKind::Tensor => (
                    "emath_rt::DenseLayout::Tensor { shape, .. }",
                    "emath_rt::Tensor { shape: shape.clone(), data }",
                ),
                _ => {
                    return Err(BackendError::UnsupportedType(
                        "dense-repack layout has a nonnumeric carrier".into(),
                    ));
                }
            };
            Ok(map_runtime_result(format!(
                "(|| -> Result<_, String> {{ let template = &{template_code}; let data = {data_code}; if data.len() != template.len() {{ return Err(String::from(\"numeric storage does not match template\")); }} match template {{ {pattern} => Ok({result}), _ => Err(String::from(\"numeric layout has the wrong carrier\")) }} }})()"
            )))
        }
        EmirOp::IndexText(value) => Ok(Expr::Raw(format!(
            "({} as usize).to_string()",
            render_expr(&typed_operand(program, *value, ValueKind::F64, kinds))
        ))),
        EmirOp::F64Exp2(value) => Ok(Expr::Raw(format!(
            "({}).exp2()",
            render_expr(&typed_operand(program, *value, ValueKind::F64, kinds))
        ))),
        EmirOp::F64PowI(base, exponent) => {
            if kind_at(kinds, *exponent) != ValueKind::I64 {
                return Err(BackendError::UnsupportedType(
                    "powi exponent must be Int".into(),
                ));
            }
            Ok(Expr::Raw(format!(
                "({}).powi(i32::try_from({}).map_err(|_| String::from(\"E-SCALAR-CONVERT: exponent exceeds i32\"))?)",
                render_expr(&typed_operand(program, *base, ValueKind::F64, kinds)),
                render_expr(&operand(program, *exponent))
            )))
        }
        EmirOp::TextTrim(value) | EmirOp::TextLength(value) | EmirOp::ParseF64(value) => {
            if kind_at(kinds, *value) != ValueKind::Text {
                return Err(BackendError::UnsupportedType(
                    "text operation requires Text".into(),
                ));
            }
            let text = render_expr(&operand(program, *value));
            Ok(Expr::Raw(match op {
                EmirOp::TextTrim(_) => format!("({text}).trim().to_owned()"),
                EmirOp::TextLength(_) => format!(
                    "i64::try_from(({text}).len()).map_err(|_| String::from(\"text length exceeds Int\"))?"
                ),
                _ => format!(
                    "({text}).parse::<f64>().map_err(|_| String::from(\"E-SCALAR-CONVERT: invalid Float64 text\"))?"
                ),
            }))
        }
        EmirOp::TextByte(text, index) => {
            if kind_at(kinds, *text) != ValueKind::Text {
                return Err(BackendError::UnsupportedType(
                    "text_byte requires Text".into(),
                ));
            }
            let text = render_expr(&operand(program, *text));
            let index = render_expr(&checked_integer_operand(program, *index, kinds)?);
            Ok(Expr::Raw(format!(
                "{{ let __byte_index = usize::try_from({index}).map_err(|_| String::from(\"text byte index must be nonnegative\"))?; i64::from(*({text}).as_bytes().get(__byte_index).ok_or_else(|| String::from(\"text byte index out of bounds\"))?) }}"
            )))
        }
        EmirOp::FormatScientific(value, precision) => {
            if kind_at(kinds, *precision) != ValueKind::I64 {
                return Err(BackendError::UnsupportedType(
                    "format precision must be Int".into(),
                ));
            }
            Ok(map_runtime_result(format!(
                "emath_rt::format_scientific({}, {})",
                render_expr(&typed_operand(program, *value, ValueKind::F64, kinds)),
                render_expr(&operand(program, *precision))
            )))
        }
        EmirOp::Select {
            condition,
        ...
        EmirOp::MatrixCreate {
            ...
            let values = elements
                ...
                .join(", ");
            Ok(map_runtime_result(format!(
                "emath_rt::Matrix::new({rows}, {cols}, vec![{values}])"
            )))
        }
        EmirOp::MatrixRows(value) | EmirOp::MatrixCols(value) => {
            let matrix = render_expr(&operand(program, *value));
            let dimension = if matches!(op, EmirOp::MatrixRows(_)) {
                "rows"
            } else {
                "cols"
            };
            Ok(map_runtime_result(format!(
                r#"i64::try_from(({matrix}).{dimension}()).map_err(|_| "matrix dimension exceeds Int")"#
            )))
        }
        EmirOp::MatrixPack { rows, cols, data } => {
            ...
            Ok(map_runtime_result(format!(
                r#"(|| -> Result<emath_rt::Matrix, String> {{
        ...
        EmirOp::TensorShape(value) => {
            ...
            Ok(map_runtime_result(format!(
                r#"{{ let tensor = &({value}); if tensor.shape.iter().try_fold(1usize, |size, axis| size.checked_mul(*axis)) != Some(tensor.data.len()) {{ Err("tensor storage does not match shape") }} else {{ tensor.shape.iter().map(|axis| i64::try_from(*axis).map_err(|_| "tensor dimension exceeds Int")).collect::<Result<Vec<i64>, _>>() }} }}"#
            )))
        }
        EmirOp::TensorPack { shape, data } => {
            if kind_at(kinds, *shape) != ValueKind::Vector(Box::new(ValueKind::I64))
                || kind_at(kinds, *data) != ValueKind::Vector(Box::new(ValueKind::F64))
            {
                return Err(BackendError::UnsupportedType(
                    "tensor packing requires Int dimensions and Float64 data".into(),
                ));
            }
            ...
            let data = render_expr(&owned_operand(program, *data, kinds));
            Ok(map_runtime_result(format!(
                r#"(|| -> Result<emath_rt::Tensor, String> {{
        ...
        EmirOp::DenseIndex { dense, index } => {
            ...
            let (storage, validation) = match kind_at(kinds, *dense) {
                ValueKind::Vector(element) if *element == ValueKind::F64 => {
                    (format!("({value}).as_slice()"), String::new())
                }
                ValueKind::Matrix(element) if *element == ValueKind::F64 => {
                    (format!("({value}).as_slice()"), String::new())
                }
                ValueKind::Tensor => (
                    format!("({value}).data.as_slice()"),
                    format!(
                        r#"if ({value}).shape.iter().try_fold(1usize, |size, axis| size.checked_mul(*axis)) != Some(({value}).data.len()) {{ return Err("tensor storage does not match shape"); }}"#
                    ),
                ),
                _ => {
                    return Err(BackendError::UnsupportedType(
                        "dense indexing requires a Float64 dense carrier".into(),
                    ));
                }
            };
            let index = render_expr(&operand(program, *index));
            Ok(map_runtime_result(format!(
                r#"(|| -> Result<f64, &str> {{ {validation} usize::try_from({index}).ok().and_then(|index| {storage}.get(index)).copied().ok_or("dense index out of bounds") }})()"#
            )))
        }
        ...
        EmirOp::VectorIndex { vector, index } => {
            ...
            if kind_at(kinds, *index) == ValueKind::I64 {
                ...
                Ok(map_runtime_result(format!(
                    "{{ let index = {index}; if index.is_finite() && index >= 0.0 && index.fract() == 0.0 {{ ({collection}).get(index as usize){materialize}.ok_or(\"vector index out of bounds\") }} else {{ Err(\"vector index must be a nonnegative integer\") }} }}"
                )))
            }
        }
...
pub(super) fn dense_point_index_expr(
    args: &[EmirValue],
    program: &EmirProgram,
    kinds: &[ValueKind],
) -> Option<Expr> {
    let [dense, coordinates] = args else {
        return None;
    };
    let (shape, data) = match kind_at(kinds, *dense) {
        ValueKind::Vector(element) if *element == ValueKind::F64 => {
            ("&[__dense.len()][..]", "__dense.as_slice()")
        }
        ValueKind::Matrix(element) if *element == ValueKind::F64 => (
            "&[__dense.rows(), __dense.cols()][..]",
            "__dense.as_slice()",
        ),
...
fn layout_match_code(layout: &str, value: &str, kind: &ValueKind) -> String {
    match kind {
        ValueKind::F64 | ValueKind::I64 => {
            format!("matches!(&({layout}), emath_rt::DenseLayout::Scalar)")
        }
        ValueKind::Vector(element) if **element == ValueKind::F64 => format!(
            "matches!(&({layout}), emath_rt::DenseLayout::Vector(len) if *len == ({value}).len())"
        ),
        ValueKind::Matrix(element) if **element == ValueKind::F64 => format!(
            "matches!(&({layout}), emath_rt::DenseLayout::Matrix {{ rows, cols, len }} if *rows == ({value}).rows() && *cols == ({value}).cols() && *len == ({value}).as_slice().len())"
        ),
        ValueKind::Tensor => format!(
            "matches!(&({layout}), emath_rt::DenseLayout::Tensor {{ shape, len }} if shape == &({value}).shape && *len == ({value}).data.len())"
        ),
        _ => "false".into(),
    }
}
