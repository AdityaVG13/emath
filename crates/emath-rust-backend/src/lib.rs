...
mod tests {
    ...
    use emath_core::Span;
    use emath_ir::{BinaryOp, ExprNode, Literal};

    fn square_package() -> SemanticPackage {
        let mut package = SemanticPackage::new();
        package.push_expr(
            ExprNode::Variable(QualifiedName("x".to_string())),
            Span::default(),
        );
        package
    }
    ...
    fn temper_smoke() {
        ...
        assert!(output.files["src/lib.rs"].contains("#![forbid(unsafe_code)]"));
    }

    #[test]
    fn op_expr_matches_semantics() {
        // x * x
        let mut package = SemanticPackage::new();
