...
/// exactly as the codegen match does. The public `BackendInput::generate`
/// surface cannot yet reach the `int_nullspace`/`exact_product_delta`
/// arms because `emath_exec_ir::emitter::call` does not register those
/// names (exec-ir ownership), so those integration tests drive `op_expr`
/// through this zero-semantics delegate. The aj8d option/result carrier
/// ops ARE reachable end-to-end through `BackendInput::generate`
/// (emitter registers all nine names), so pass-5 coverage uses the real
/// path, not this seam. No behavior of its own.
#[doc(hidden)]
pub fn render_op_expr_for_tests(
    op: &EmirOp,
    program: &EmirProgram,
    names: &[String],
    states: &[String],
    ...
    op_expr(op, program, names, states, i64_names)
}
...
impl BackendInput<'_> {
    ...
    /// for nested carriers (emath-option-result-graph-field-aj8d pass 4).
    fn rust_output_ty(&self, ty: TypeId, owner: &str) -> Result<Ty, BackendError> {
        let Some(node) = self.package.ty(ty) else {
            return Err(BackendError::UnsupportedType(format!(
                "unknown type id in `{owner}`"
            )));
    ...
    fn rust_output_node(&self, node: &TypeNode, owner: &str) -> Result<Ty, BackendError> {
        match node {
