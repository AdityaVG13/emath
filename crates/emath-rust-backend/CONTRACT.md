...
- Program-space quote emission (emath-npky7): `CodeLiteral` renders as a two-stage factory - the nested program lowers once into `move |param: ExactRatio, free...: ExactRatio| -> Result<ExactRatio, String>`, and `emath_rt::code::open` wraps it in the outer factory (`Rc<dyn Fn(&[ExactRatio]) -> Result<UnaryRat, String>>`) that owns the bound constants and yields the specialized closure (`ValueKind::Code`). `CodeSubstitute` renders `emath_rt::code::substitute(&code, "name", value)` (a Rational value; anything else is `UnsupportedType`); `CodeEvaluate` renders `emath_rt::code::evaluate(&code)?` with kind `Closure { Rational -> Rational }` so `CallValue` applies it typed. Hygiene is structural: the nested body's only external names are its parameter and the open constants, so there is no ambient capture to render; no tree and no interpreter are emitted - candidates execute as the compiled closures.
