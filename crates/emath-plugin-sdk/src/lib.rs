...
mod tests {
    ...
    fn simple() -> PluginDescriptor {
        descriptor_for(
            ...
            SandboxPolicy {
                ...
                permissions: vec![],
                network: false,
                allowed_capabilities: vec!["codegen".into()],
            },
        )
    }

    #[test]
    fn local_well_scoped_plugin_is_admitted() {
        assert!(admit(&simple(), Trust::Local).is_ok());
        assert!(admit(&simple(), Trust::Untrusted).is_ok());
    }

    #[test]
    fn capability_outside_allowed_set_is_refused() {
        let mut plugin = simple();
        plugin.capabilities.push("exec".into());
        let error = admit(&plugin, Trust::Local).unwrap_err();
        assert_eq!(error.code, "E-PLG-003");
    }

    #[test]
    fn empty_capabilities_are_refused() {
        let mut plugin = simple();
    ...
    #[test]
    fn network_without_permission_is_refused() {
        let mut plugin = simple();
        plugin.sandbox.network = true;
        let error = admit(&plugin, Trust::Local).unwrap_err();
        assert_eq!(error.code, "E-PLG-002");
    }

    #[test]
    fn network_with_permission_is_admitted() {
        let mut plugin = simple();
        plugin.sandbox.network = true;
        plugin.sandbox.permissions.push("network".into());
        assert!(admit(&plugin, Trust::Local).is_ok());
    }

    #[test]
    fn untrusted_without_fuel_is_refused() {
        let mut plugin = simple();
        plugin.sandbox.fuel = None;
        assert!(admit(&plugin, Trust::Local).is_ok());
        let error = admit(&plugin, Trust::Untrusted).unwrap_err();
        assert_eq!(error.code, "E-PLG-002");
        plugin.sandbox.fuel = Some(0);
        let error = admit(&plugin, Trust::Untrusted).unwrap_err();
        assert_eq!(error.code, "E-PLG-002");
    }

    #[test]
    fn execution_is_typed_refusal_in_phase_one() {
        let error = execute(&simple(), b"input").unwrap_err();
        assert_eq!(error.code, "E-PLG-001");
    }

    #[test]
    fn execution_never_bypasses_admission() {
        let mut plugin = simple();
        plugin.sandbox.network = true; // refused by admit
        let error = execute(&plugin, b"input").unwrap_err();
        assert_eq!(error.code, "E-PLG-002");
    }

    #[test]
    fn interface_compat_positive_and_negative() {
        assert!(compatible(&simple(), INTERFACE_CORE).is_ok());
        let error = compatible(&simple(), "emath.plugin.interface.v2").unwrap_err();
        assert_eq!(error.code, "E-PLG-004");
    }

    #[test]
    fn canonical_json_and_content_id_are_deterministic() {
        let first = simple();
        let second = simple();
        ...
        assert_eq!(
            first.content_id(),
            content_id_of_str(&first.canonical_json()).0
        );
    }

    #[test]
    fn reordered_capabilities_change_the_descriptor() {
        let mut plugin = simple();
        plugin.capabilities = vec!["codegen".into(), "locality".into()];
        let other = descriptor_for(
            "scalar.native",
            "dew.rust",
            vec!["locality".into(), "codegen".into()],
            plugin.sandbox.clone(),
        );
        assert_ne!(plugin.canonical_json(), other.canonical_json());
    }
}
