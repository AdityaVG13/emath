...
mod tests {
    ...
    use crate::structural::{
        Component, ComponentKind, EqExpr, Equation, StructuralModel, Unit, VariableDecl,
        VariableKind,
    };
    use emath_ir::TypeNode;

    fn locked_seam() -> AdapterSeam {
        AdapterSeam {
            provider: "rumoca".into(),
            version: ProviderVersion {
                major: 3,
                minor: 8,
                patch: 0,
            },
            capability: "structural-plan-v1".into(),
            content_identity: 42,
        }
    }

    #[test]
    fn identical_seam_verifies() {
        assert!(locked_seam().verify(&locked_seam()).is_ok());
    }

    #[test]
    fn version_drift_is_typed_error() {
        let mut observed = locked_seam();
        observed.version.patch = 1;
        let error = observed.verify(&locked_seam()).unwrap_err();
        assert_eq!(error.code(), "E-PROV-401");
    }

    #[test]
    fn identity_mismatch_is_typed_error() {
        let mut observed = locked_seam();
        observed.capability = "other-schema".into();
        let error = observed.verify(&locked_seam()).unwrap_err();
        assert_eq!(error.code(), "E-PROV-402");
    }

    #[test]
    fn provider_output_is_gated() {
        let seam = locked_seam();
        let broken = StructuralModel {
        ...
        let issues = seam.gate_provider_output(&broken);
        assert!(!issues.is_empty());
        assert!(issues.iter().any(|issue| issue.code == "E-TYPE-101"));
    }
}
