...
mod tests {
    ...
    use crate::structural::{
        Component, ComponentKind, EqExpr, Equation, Event, InitialCondition, StructuralModel, Unit,
    ...
    #[test]
    fn supported_subset_passes() {
        let issues = check(&mass_spring());
        assert!(issues.is_empty(), "unexpected issues: {issues:?}");
    }

    #[test]
    fn discrete_event_is_refused() {
        let mut model = mass_spring();
        model.events[0].continuous = false;
        let issues = check(&model);
        assert_eq!(issues[0].code, "E-KIND-310");
    }
}
