        ...
        match message.method.as_str() {
            "initialize" => {
                let capabilities = JsonValue::Object(
                    [
                        // Positions are UTF-8 byte offsets, so the server
                        // must advertise the encoding it actually uses
                        // (SURF-0009); a glyph-bearing document would
                        // otherwise be edited at wrong offsets by
                        // UTF-16-counting clients.
                        ("positionEncoding".into(), JsonValue::String("utf-8".into())),
                        (
...
mod tests {
    ...
    /// `⋈` occupies three character units, not one (SURF-0009).
    #[test]
    fn range_offsets_use_utf8_byte_characters_on_glyph_lines() {
        let text = "ab ⋈ cd\n";
        // a(0) b(1) ' '(2) ⋈(3..5) ' '(6) c(7) d(8)
        let range = JsonValue::Object(BTreeMap::from([
            (
                "start".into(),
        ...
        assert_eq!(range_offsets(&range, text), Some((7, 8)));
        // Reverse mapping is byte-exact: offset 7 sits at `c`, whose
        // UTF-8 byte index is also 7 — a UTF-16 encoding would report 6.
        assert_eq!(
            offset_range(text, 7),
            (
                Position {
