...
mod tests {
    ...
    fn msg(id: Option<i64>, method: &str, params: JsonValue) -> RpcMessage {
        RpcMessage {
            id,
            method: method.into(),
            params,
    ...
    fn run_messages(messages: &[RpcMessage]) -> String {
        let mut state = ServerState::new();
        let mut output = Vec::new();
        for message in messages {
            state.handle(message, &mut output).unwrap();
        }
        String::from_utf8(output).unwrap()
    }

    const VALID: &str = "emath custom <Square> as function:\n    inputs:\n        x: Float64\n    outputs:\n        y: Float64\n    definitions:\n        y = x * x\n";

    #[test]
    fn initialize_reports_capabilities() {
        let out = run_messages(&[msg(Some(1), "initialize", JsonValue::Object([].into()))]);
        assert!(out.contains("\"capabilities\""));
        assert!(out.contains("\"textDocumentSync\":2"));
        assert!(out.contains("\"serverInfo\""));
        assert!(out.contains("\"emath-lsp\""));
    }

    #[test]
    fn did_open_valid_document_publishes_no_diagnostics() {
        let params = JsonValue::Object(
            [(
                "textDocument".into(),
                JsonValue::Object(
                    [
                        (
                            "uri".into(),
                            JsonValue::String("file:///tmp/a.emath".into()),
                        ),
                        ("text".into(), JsonValue::String(VALID.into())),
                    ]
                    .into(),
                ),
            )]
        ...
        let out = run_messages(&[msg(None, "textDocument/didOpen", params)]);
        assert!(out.contains("textDocument/publishDiagnostics"));
        assert!(out.contains("\"diagnostics\":[]"));
    }

    #[test]
    fn did_open_invalid_document_publishes_typed_diagnostics() {
        let invalid = "emath custom <Dup> as function:\n    inputs:\n        x: Float64\n        x: Float64\n    outputs:\n        y: Float64\n    definitions:\n        y = x\n";
        let params = JsonValue::Object(
            [(
                "textDocument".into(),
                JsonValue::Object(
                    [
                        (
                            "uri".into(),
                            JsonValue::String("file:///tmp/dup.emath".into()),
                        ),
                        ("text".into(), JsonValue::String(invalid.into())),
                    ]
                    .into(),
                ),
            )]
        ...
        assert!(out.contains("E-NAME-020"));
    }
    ...
    fn incremental_change_updates_diagnostics() {
                ...
                (
                    ...
                    JsonValue::Object(
                        [
                            ...
                            ("version".into(), JsonValue::Number(2)),
                        ]
                        .into(),
                    ),
                ),
                                    ...
                                    [
                                        ...
                                        (
                                            ...
                                            JsonValue::Object(
                                                [
                                                    ("line".into(), JsonValue::Number(2)),
                                                    ("character".into(), JsonValue::Number(17)),
                                                ]
                                                .into(),
                                            ),
                                        ),
        ...
        let out = run_messages(&[
            msg(None, "textDocument/didOpen", open),
            msg(None, "textDocument/didChange", change),
        ]);
        // Open publishes an empty list; the change introduces an unknown
        // variable and must re-publish a typed diagnostic.
        assert!(out.contains("\"diagnostics\":[]"), "open publish: {out}");
        assert!(out.contains("\"code\":\"E-"), "change publish: {out}");
        assert!(
            out.matches("textDocument/publishDiagnostics").count() >= 2,
            "expected a re-publish after the change"
        );
    }

    #[test]
    fn did_change_applies_range_edit() {
        let mut state = ServerState::new();
        let open = JsonValue::Object(
            [(
                "textDocument".into(),
        ...
        state
            .handle(&msg(None, "textDocument/didOpen", open), &mut Vec::new())
            .unwrap();
        let change = JsonValue::Object(
            [
                (
                                ...
                                JsonValue::Object(
                                    [
                                        (
                                            ...
                                            JsonValue::Object(
                                                [
                                                    ("line".into(), JsonValue::Number(6)),
                                                    ("character".into(), JsonValue::Number(12)),
                                                ]
                                                .into(),
                                            ),
                                        ),
        ...
        state
            ...
            .unwrap();
        let text = &state.documents["file:///tmp/a.emath"].text;
        assert!(text.contains("y = x *"), "text after change: {text}");
        assert!(!text.contains("y = x * x"), "change not applied: {text}");
    }

    #[test]
    fn completion_lists_keywords_deterministically() {
        let first = run_messages(&[msg(Some(3), "textDocument/completion", JsonValue::Null)]);
        let second = run_messages(&[msg(Some(3), "textDocument/completion", JsonValue::Null)]);
        assert_eq!(first, second);
        assert!(first.contains("\"emath\""));
        assert!(first.contains("\"kind\":14"));
    }

    #[test]
    fn hover_explains_known_keyword() {
        let open = JsonValue::Object(
            [(
                "textDocument".into(),
                JsonValue::Object(
                ...
                (
                    ...
                    JsonValue::Object(
                        [
                            ...
                            ("character".into(), JsonValue::Number(2)),
                        ]
                        .into(),
                    ),
                ),
        ...
        let out = run_messages(&[
            msg(None, "textDocument/didOpen", open),
            msg(Some(4), "textDocument/hover", hover),
        ]);
        assert!(out.contains("**emath**"));
    }

    #[test]
    fn unknown_method_is_json_rpc_error() {
        let out = run_messages(&[msg(Some(9), "textDocument/definition", JsonValue::Null)]);
        assert!(out.contains("\"code\":-32601"));
        assert!(out.contains("method not found"));
    }

    #[test]
    fn shutdown_then_exit_is_clean_pipe() {
        let out = run_messages(&[
            msg(Some(1), "shutdown", JsonValue::Null),
            msg(None, "exit", JsonValue::Null),
        ]);
        assert!(out.is_empty() || out.contains("\"result\":null"));
    }
}
