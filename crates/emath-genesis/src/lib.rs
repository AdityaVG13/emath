...
mod tests {
    ...
    #[test]
    fn evaluates_reference_term_in_three_worlds() {
        let (signature, term) = reference_alien_term();
        signature.validate(&term).unwrap();

        let mut symbolic_environment = Environment::new();
        symbolic_environment.insert(
            VariableId("a".into()),
            Term::Variable(VariableId("a".into())),
        );
        symbolic_environment.insert(
        ...
        let symbolic = evaluate(&term, &FreeTermWorld, &symbolic_environment).unwrap();
        assert_eq!(symbolic, term);

        let mut bool_environment = Environment::new();
        bool_environment.insert(VariableId("a".into()), true);
        bool_environment.insert(VariableId("b".into()), false);
        assert!(!evaluate(&term, &BooleanAlienWorld, &bool_environment).unwrap());

        let mut modular_environment = Environment::new();
        modular_environment.insert(VariableId("a".into()), 4);
        modular_environment.insert(VariableId("b".into()), 7);
        assert_eq!(
            evaluate(&term, &ModularAlienWorld, &modular_environment).unwrap(),
            6
        );
    }

    #[test]
    fn free_world_identity_is_stable() {
        let (signature, _) = reference_alien_term();
        let first = free_symbolic_world("AlienFree", signature.clone()).identity();
        let second = free_symbolic_world("AlienFree", signature).identity();
