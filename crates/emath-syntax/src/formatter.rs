...
mod tests {
    ...
    fn round_trip(source: &str) -> String {
        ...
        let (_, _, comments) = lex_with_comments(source, emath_core::FileId(0), &Limits::default());
        format(&tree, &comments)
    }

    /// Structural shape of a tree: the Debug form with every span erased.
    /// Round-trip stability is position-independent: formatting moves
    /// bytes but must preserve structure, names, and literal spellings.
    fn shape(tree: &SyntaxTree) -> String {
        ...
        let mut i = 0;
        while i < bytes.len() {
            if debug[i..].starts_with("Span {") {
                // Erase the whole Span structure: advance to the closing
                // brace.
                let mut depth = 0;
                while i < bytes.len() {
                    match bytes[i] {
    ...
    fn policy(definitions: &str) -> String {
        format!("emath custom <P> as policy:\n    definitions:\n        {definitions}\n")
    }
    ...
    fn formatter_is_parse_stable() {
        let sources: Vec<String> = vec![
            ...
            policy("y = (1 + 2) * 3"),
            policy("z = a ^ b ^ c"),
            policy("w = not (a < b) and (c >= d)"),
            "emath custom <P> as policy:\n    inputs:\n        x: Float64\n    definitions:\n        score = x * 2\n"
                .to_string(),
            policy("q = sum i in 1 .. 3: i * i"),
        ];
        for source in &sources {
            let once = round_trip(source);
            let twice = round_trip(&once);
            assert_eq!(once, twice, "not idempotent for {source:?}");
            // parse(format(x)) == parse(x) structurally (spans move).
            let (t1, d1) = parse_str(source);
            let (t2, d2) = parse_str(&once);
            assert!(d1.is_empty() && d2.is_empty(), "diagnostics for {source:?}");
            assert_eq!(shape(&t1), shape(&t2), "tree changed for {source:?}");
        }
    }

    #[test]
    fn parens_preserve_semantics() {
        let source = policy("x = (1 + 2) * 3");
        let formatted = round_trip(&source);
        assert!(formatted.contains("(1 + 2) * 3"), "got: {formatted}");
        let source = policy("x = 1 + 2 * 3");
        let formatted = round_trip(&source);
        assert!(
            !formatted.contains("(1 + 2)"),
            "over-parenthesized: {formatted}"
        );
    }
    ...
    fn comments_are_retained() {
        let source = "# head\nemath custom <P> as policy:\n    inputs:\n        # lead\n        x: Float64  # trailing\n";
        let formatted = round_trip(source);
        assert!(formatted.contains("# head"), "lost head: {formatted}");
        assert!(formatted.contains("# lead"), "lost lead: {formatted}");
        assert!(
            formatted.contains("# trailing"),
            "lost trailing: {formatted}"
        );
    }

    #[test]
    fn corpus_round_trips() {
        // The official corpus: templates, examples, valid fixtures.
        let corpus = [
            (
                "language/templates/minimal.emath",
                include_str!("../../../language/templates/minimal.emath"),
            ),
            (
                "language/examples/00_hello_square.emath",
                include_str!("../../../language/examples/00_hello_square.emath"),
            ),
            (
                "implementation/tests/valid/stateful.emath",
                include_str!("../../../implementation/tests/valid/stateful.emath"),
            ),
            (
                "implementation/tests/valid/minimal.emath",
                include_str!("../../../implementation/tests/valid/minimal.emath"),
            ),
        ];
        for (name, source) in corpus {
            let formatted = round_trip(source);
            let (t1, d1) = parse_str(source);
            let (t2, d2) = parse_str(&formatted);
            assert!(d1.is_empty() && d2.is_empty(), "{name}: parse failed");
            assert_eq!(
                shape(&t1),
                shape(&t2),
                "{name}: round-trip changed the tree"
            );
            // Idempotence on the corpus.
            let twice = round_trip(&formatted);
            assert_eq!(formatted, twice, "{name}: not idempotent");
        }
    }

    #[test]
    fn canonical_tokenization_is_stable() {
        let source = include_str!("../../../implementation/tests/valid/stateful.emath");
        let (a, da) = crate::lexer::lex(source, emath_core::FileId(0), &Limits::default());
        let (b, db) = crate::lexer::lex(source, emath_core::FileId(0), &Limits::default());
        assert!(da.is_empty() && db.is_empty());
        assert_eq!(a, b, "tokenization not deterministic");
    }

    #[test]
    fn quantity_and_at_survive() {
        let source = policy("x = 1 ms + 2 ms\n        u = temperature at time.start\n");
        let formatted = round_trip(&source);
        assert!(formatted.contains("2 ms"), "{formatted}");
        let (t1, _) = parse_str(&source);
        let (t2, _) = parse_str(&formatted);
        assert_eq!(shape(&t1), shape(&t2));
        // Idempotence on the quantity form.
        let twice = round_trip(&formatted);
        assert_eq!(formatted, twice);
        let _ = ExprKind::Quantity {
            value: Box::new(Expr {
