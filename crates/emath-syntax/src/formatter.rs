...
mod tests {
    ...
    fn formatting_is_idempotent_and_parse_stable() {
        ...
        let rebound = parse_lossless(&once, emath_core::FileId(0), &Limits::default());
        assert!(
    ...
    fn corpus_files_are_lossless_round_trip() {
        for (name, text) in [
            ("square", include_str!("../../../tests/valid/square.emath")),
            (
                "affine_scorer",
                include_str!("../../../tests/valid/affine_scorer.emath"),
            ),
        ] {
            let parsed = parse_lossless(text, emath_core::FileId(0), &Limits::default());
            assert!(
    ...
    fn corpus_canonical_reparse_is_stable() {
        for (name, text) in [
            ...
            let parsed = parse_lossless(text, emath_core::FileId(0), &Limits::default());
            let canonical = format(&parsed.tree, &parsed.comments);
            let reborn = parse_lossless(&canonical, emath_core::FileId(0), &Limits::default());
            assert!(
