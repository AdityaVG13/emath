...
impl Parser {
    ...
    /// `emath custom <Name<Params>> as kind:` `suite`
    fn parse_declaration(&mut self) -> Option<Declaration> {
        let start = self.current_span();
        self.advance(); // `emath`
                        // The declaration kind is a keyword-lexed word in the corpus
                        // (`custom`); accept both spellings.
        ...
        if !self.eat(&TokenKind::Lt) {
            self.error_here("E-SYN-101", "expected `<Name>` after declaration kind");
            return None;
        }
    ...
    fn parse_loose_expr(&mut self) -> Option<Expr> {
        ...
        } = &expr.kind
        {
            ...
            while let TokenKind::Ident(next) = self.peek().clone() {
                if matches!(self.peek_at(1), TokenKind::Ident(_) | TokenKind::Newline) {
                    segments.push(next);
                    self.advance();
                    joined = true;
                } else {
    ...
    fn parse_default_ident_statement(&mut self, start: Span) -> Option<Stmt> {
        ...
        // assignments with indexed targets: `norm[b, t] = ...`
        if matches!(self.peek_at(1), TokenKind::LBracket) {
            ...
            self.error_here("E-SYN-101", "expected `=` after indexed target");
            return None;
        }
        ...
        while index < max {
            match &self.tokens[index].kind {
                ...
                TokenKind::RParen | TokenKind::RBracket => {
                    ...
                    if depth == 0 {
                        // after the matching paren: arrow or colon => header
                        return matches!(
                            self.tokens.get(index + 1).map(|t| &t.kind),
                            Some(TokenKind::Arrow | TokenKind::Colon)
                        ) || self.tokens.get(index + 1).map(|t| &t.kind)
    ...
    /// Collect identifiers optionally joined by `.`. Returns segments and
    /// whether a dot joined any of them.
    ...
    fn parse_command_tail(
        ...
        loop {
            match self.peek().clone() {
                TokenKind::Ident(next) => {
                    ...
                    // A dotted path or parenthesized expression starts an
                    // argument, so the word is left unconsumed there.
                    if matches!(
                        self.peek_at(1),
                        TokenKind::Ident(_)
                            | TokenKind::Keyword(
                ...
                TokenKind::Keyword(
                    Keyword::Against | Keyword::Where | Keyword::Over | Keyword::With,
                ) => {
        ...
        // `numeric strict-f64` / `safety forbid-unsafe`
