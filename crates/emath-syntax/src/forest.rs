...
mod tests {
    ...
    const BODY: &str = "⧖(a ⋈ b) ⊛ ζ";
    const EXPECTED_CANONICAL: &str = "apply(⊛,apply(⧖,apply(⋈,var(a),var(b))),const(ζ))";
    ...
    #[test]
    fn reference_body_exit() {
        let limits = ForestLimits::default();
        let forest = build_forest_named(BODY, "AlienGlyphs", &limits);
        assert_eq!(forest.ambiguity_count(), 1);
        assert!(forest.holes().is_empty());
        let term = forest.unique_term().unwrap();
        assert_eq!(term.canonical(), EXPECTED_CANONICAL);

        let inference = infer_signature_named(BODY, "AlienGlyphs", &limits).unwrap();
        assert_eq!(inference.signature, reference_signature());
        assert_eq!(inference.signature.arity(&SymbolId("⧖".into())), Some(1));
        let fixity = |symbol: &str| inference.fixities.get(&SymbolId(symbol.into()));
        assert_eq!(fixity("⧖"), Some(&Fixity::Prefix));
        assert_eq!(fixity("⋈"), Some(&Fixity::Infix));
        assert_eq!(fixity("⊛"), Some(&Fixity::Infix));
        assert_eq!(fixity("ζ"), Some(&Fixity::Constant));
        assert_eq!(
            inference.variables,
            vec![VariableId("a".into()), VariableId("b".into())]
        );
        assert_eq!(inference.type_variables.len(), 4);
    }

    #[test]
    fn determinism_two_runs_byte_identical() {
        let limits = ForestLimits::default();
        let first = build_forest_named(BODY, "AlienGlyphs", &limits);
        let second = build_forest_named(BODY, "AlienGlyphs", &limits);
        assert_eq!(first.canonical_json(), second.canonical_json());
        assert_eq!(first.parse_id(), second.parse_id());
        assert_eq!(first.node_count(), second.node_count());

        let first_inference = infer_signature_named(BODY, "AlienGlyphs", &limits).unwrap();
        let second_inference = infer_signature_named(BODY, "AlienGlyphs", &limits).unwrap();
        assert_eq!(
            first_inference.canonical_json(),
            second_inference.canonical_json()
        );
        assert_eq!(
            first_inference.signature_id(),
            second_inference.signature_id()
        );
    }

    #[test]
    fn canonical_json_carries_glyphs_and_schema() {
        let forest = build_forest_named(BODY, "AlienGlyphs", &ForestLimits::default());
        let json = forest.canonical_json();
        assert!(json.starts_with("{\"schema\":\"emath.parse-forest.v1\""));
        assert!(json.contains("\"world_name\":\"AlienGlyphs\""));
        assert!(json.contains(&format!("\"body\":\"{BODY}\"")));
        assert!(json.contains(&format!("\"canonical_term\":\"{EXPECTED_CANONICAL}\"")));
        assert!(json.ends_with("\"recovery\":\"bounded-holes\"}"));
    }

    #[test]
    fn recovery_hole_on_unparseable_body() {
        let forest = build_forest("a ⋈", &ForestLimits::default());
        assert_eq!(forest.ambiguity_count(), 0);
        let holes = forest.holes();
        assert_eq!(holes.len(), 1);
        assert_eq!(holes[0].1, "unparseable-body");
        let error = forest.unique_term().unwrap_err();
        assert_eq!(error.code, "E-SYN-210");
        assert_eq!(
            infer_signature("a ⋈", &ForestLimits::default()).unwrap_err()[0].code,
            "E-SYN-210"
        );
    }

    #[test]
    fn budget_caps_respected() {
        ...
        let first = build_forest_named(BODY, "AlienGlyphs", &limits);
        assert!(first.node_count() <= limits.max_nodes);
        assert!(first.ambiguity_count() <= limits.max_alternatives);
        assert_eq!(first.holes()[0].1, "node-budget");
        assert_eq!(first.unique_term().unwrap_err().code, "E-SYN-210");
    ...
    fn ambiguous_without_precedence() {
        let forest = build_forest("a ⋈ b ⊛ c", &ForestLimits::default());
        assert_eq!(forest.ambiguity_count(), 2);
        assert!(forest.holes().is_empty());
        let error = forest.unique_term().unwrap_err();
        assert_eq!(error.code, "E-SYN-211");
        assert_eq!(
            infer_signature("a ⋈ b ⊛ c", &ForestLimits::default()).unwrap_err()[0].code,
            "E-SYN-211"
        );
    }

    #[test]
    fn prefix_application_with_multiple_arguments() {
        let forest = build_forest("f(a, b)", &ForestLimits::default());
        assert_eq!(forest.ambiguity_count(), 1);
        let term = forest.unique_term().unwrap();
        assert_eq!(term.canonical(), "apply(f,var(a),var(b))");
        let inference = infer_signature("f(a, b)", &ForestLimits::default()).unwrap();
        assert_eq!(inference.signature.arity(&SymbolId("f".into())), Some(2));
        assert_eq!(
            inference.fixities.get(&SymbolId("f".into())),
            Some(&Fixity::Prefix)
        );
    }
}
