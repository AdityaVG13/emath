...
mod tests {
    use super::*;
    use crate::artifact_check::tests_shared::sample_input;

    #[test]
    fn every_standard_control_is_refused_by_the_independent_checker() {
        let input = sample_input();
        let run = run_standard_battery(&input);
        assert_eq!(run.refused.len(), 5);
        assert!(run.all_refused(), "escaped controls: {:?}", run.escaped);
    }

    #[test]
    fn a_corrected_control_never_escapes_silently() {
        let input = sample_input();
        let mut control = seed_tampered(&input);
        // Repair the tamper: the control must still be self-describing.
        control.artifact = input.clone();
        let run = run_negative_controls(
            std::slice::from_ref(&control),
            &ArtifactCheckConfig::default(),
        );
        assert!(!run.all_refused());
        assert_eq!(run.escaped[0].1, "admitted");
    }

    #[test]
    fn return_values_satisfy_the_refusal_contract() {
        let input = sample_input();
        let ok = crate::check_artifact(
            &input,
            &ArtifactCheckConfig {
                supported_claim_classes: vec!["correctness".into()],
            },
        ...
        assert!(ok.valid());
        let error =
            crate::CheckerError::new("E-EVID-105", "required artifact file missing: src/lib.rs");
        assert_eq!(
            error.code,
            NegativeControlKind::IncompleteArtifact.expected_code()
        );
    }
}
