...
mod tests {
    ...
    fn relations() -> Vec<TranslationRelation> {
        vec![
            TranslationRelation {
                label: "affine".into(),
                inputs: vec![1.0, 2.0],
    ...
    fn samples() -> Vec<TranslationSample> {
        vec![
            TranslationSample {
                label: "affine".into(),
                inputs: vec![1.0, 2.0],
    ...
    #[test]
    fn matching_relation_produces_a_verifiable_witness() {
        let witness = validate_translation(&relations(), &samples()).unwrap();
        assert!(check_witness(&witness, &relations(), &samples()).is_ok());
    }

    #[test]
    fn mutated_generated_rust_is_refused() {
        // A mutation in the generated crate flips one output.
        let mut mutated = samples();
        mutated[1].outputs = vec![14.0];
        let error = validate_translation(&relations(), &mutated).unwrap_err();
        assert_eq!(error.code, "E-EVID-301");
        assert!(error.message.contains("translation mismatch"));
    }

    #[test]
    fn missing_coverage_is_refused() {
        let partial = vec![samples()[0].clone()];
        let error = validate_translation(&relations(), &partial).unwrap_err();
        assert_eq!(error.code, "E-EVID-301");
    }

    #[test]
    fn tampered_witness_is_refused() {
        let mut witness = validate_translation(&relations(), &samples()).unwrap();
        witness.sample_footprint = format!("{:016x}", usize::MAX);
        let error = check_witness(&witness, &relations(), &samples()).unwrap_err();
        assert_eq!(error.code, "E-EVID-302");
    }

    #[test]
    fn semantic_variants_never_mix() {
        // Same label, different input vector: rows are distinct.
        let mut shifted = samples();
        shifted[0].inputs = vec![2.0, 2.0];
        let error = validate_translation(&relations(), &shifted).unwrap_err();
        assert_eq!(error.code, "E-EVID-301");
    }
}
