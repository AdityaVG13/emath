...
mod tests {
    ...
    #[test]
    fn ladder_builds_within_contracts() {
        let graph = FallbackGraph::build(&[
            ("p1".into(), "exact".into(), "rust-library".into()),
            ("p1".into(), "bounded".into(), "rust-library".into()),
            ("p1".into(), "estimate".into(), "rust-library".into()),
            ("p2".into(), "exact".into(), "python".into()),
        ]);
        assert!(graph.contracts_preserved());
        assert_eq!(
            graph
                .fallback_of("p1", "exact", "rust-library")
                .unwrap()
                .tier,
            "bounded"
        );
        assert_eq!(
            graph
                .fallback_of("p1", "bounded", "rust-library")
                .unwrap()
                .tier,
            "estimate"
        );
        assert!(graph
            .fallback_of("p1", "estimate", "rust-library")
            .is_none());
    }

    #[test]
    fn no_cross_contract_edges() {
        let graph = FallbackGraph::build(&[
            ("p1".into(), "exact".into(), "rust-library".into()),
            ("p1".into(), "estimate".into(), "python".into()),
        ]);
        assert!(graph.contracts_preserved());
