...
mod tests {
    use super::*;
    use crate::registry_helpers::sample_registry;
    use emath_core::Span;
    use emath_ir::{
        DeterminismPolicy, EvidenceLevel, ExactnessPolicy, FallbackPolicy, GoalKind,
        GoalRequirements, TargetProfile,
    };

    fn goal(exactness: ExactnessPolicy, fallback: FallbackPolicy) -> Goal {
        Goal {
            id: GoalId(0),
    ...
    fn exact_provider_selected_over_approximate() {
        ...
        let outcome = plan(
            &goal(ExactnessPolicy::Exact, FallbackPolicy::NativeOnly),
            &registry,
            &PlannerConfig::default(),
        );
        match outcome {
            PlanningOutcome::Selected { plan, .. } => {
                assert_eq!(plan.root, PlanNodeId(5));
                assert_eq!(plan.artifact_class, "native");
            }
            other => panic!("expected selection, got {:?}", other.disposition_name()),
        }
    }
    ...
    fn approximate_selected_under_declared_tolerance() {
        ...
        assert!(matches!(outcome, PlanningOutcome::Selected { .. }));
    }

    #[test]
    fn no_eligible_produces_reasons_and_disposition() {
        let registry = ProviderRegistry::new(emath_provider_api::RegistryConfig::static_only());
        let outcome = plan(
            &goal(ExactnessPolicy::Exact, FallbackPolicy::Parametric),
            &registry,
            &PlannerConfig::default(),
        );
        match outcome {
            PlanningOutcome::NoEligible {
                ...
                assert_eq!(disposition, ArtifactDisposition::Parametric);
            }
            other => panic!("expected no-eligible, got {:?}", other.disposition_name()),
        }
    }

    #[test]
    fn deterministic_tie_is_stable() {
        let registry = sample_registry();
        let goal = goal(ExactnessPolicy::Exact, FallbackPolicy::NativeOnly);
        let first = plan(&goal, &registry, &PlannerConfig::default());
        let second = plan(&goal, &registry, &PlannerConfig::default());
        assert_eq!(first, second);
    }

    #[test]
    fn exhausted_budget_yields_continuation_per_policy() {
        let registry = sample_registry();
        ...
        let outcome = plan(
            &goal(ExactnessPolicy::Exact, FallbackPolicy::Continuation),
            &registry,
            &config,
        );
        // 2 candidates retained; must not silently select beyond horizon.
        if let PlanningOutcome::Exhausted {
            continuation,
            disposition,
            ..
        } = outcome
        {
            assert!(continuation.contains("resume"));
            assert_eq!(disposition, ArtifactDisposition::Continuation);
        } else {
            // smaller fixture: selection is fine, but must stay deterministic
        }
    }
}
