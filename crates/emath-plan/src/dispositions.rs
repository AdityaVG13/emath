...
mod tests {
    ...
    #[test]
    fn every_fallback_maps_to_a_total_disposition() {
        for fallback in [
            FallbackPolicy::NativeOnly,
            FallbackPolicy::Parametric,
            FallbackPolicy::Continuation,
            FallbackPolicy::Diagnostic,
            FallbackPolicy::ExplicitLadder,
        ] {
            assert_ne!(
                disposition_without_plan(&fallback),
                ArtifactDisposition::Native,
                "no-plan must never pretend to be native"
            );
        }
    }

    #[test]
    fn exhausted_budget_per_policy() {
        assert_eq!(
            disposition_exhausted(&FallbackPolicy::Continuation),
            ArtifactDisposition::Continuation
        );
        assert_eq!(
            disposition_exhausted(&FallbackPolicy::NativeOnly),
            ArtifactDisposition::Diagnostic
        );
    }

    #[test]
    fn selected_plan_disposition_reflects_conversions() {
        assert_eq!(disposition_for_plan(false), ArtifactDisposition::Native);
        assert_eq!(disposition_for_plan(true), ArtifactDisposition::Hybrid);
    }
}
