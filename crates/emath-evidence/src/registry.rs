...
mod tests {
    ...
    #[test]
    fn registry_round_trips_all_seven_kinds() {
        let mut registry = CertificateRegistry::default();
        for kind in CertificateKind::all() {
            registry.register(contract(kind, "1.0.0")).unwrap();
        }
        assert_eq!(registry.canonical().matches("1.0.0").count(), 7);
        assert!(registry
            .admits(CertificateKind::Interval, "1.0.0", "correctness")
            .unwrap());
    }

    #[test]
    fn duplicate_versioned_contract_is_refused() {
        let mut registry = CertificateRegistry::default();
        registry
        ...
        let error = registry
            .register(contract(CertificateKind::Proof, "1.0.0"))
            .unwrap_err();
        assert_eq!(error.code, "E-EVID-402");
    }

    #[test]
    fn unknown_certificate_kind_is_refused() {
        let registry = CertificateRegistry::default();
        let error = registry
            .lookup(CertificateKind::Proof, "1.0.0")
            .unwrap_err();
        assert_eq!(error.code, "E-EVID-401");
    }

    #[test]
    fn contract_admits_only_declared_classes() {
        let mut registry = CertificateRegistry::default();
        registry
            .register(contract(CertificateKind::Rewrite, "2.1.0"))
            .unwrap();
        assert!(!registry
            .admits(CertificateKind::Rewrite, "2.1.0", "safety")
            .unwrap());
        assert!(registry
            .admits(CertificateKind::Rewrite, "2.1.0", "equivalence")
            .unwrap());
    }
}
