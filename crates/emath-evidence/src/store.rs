...
mod tests {
    ...
    #[test]
    fn content_addressing_is_idempotent_and_orders_free() {
        let mut store = EvidenceStore::default();
        let first = store.register(sample_record()).unwrap();
        let second = store.register(sample_record()).unwrap();
        assert_eq!(first, second);
        assert_eq!(store.query_resolved().len(), 1);
    }

    #[test]
    fn revoke_and_supersede_are_append_only() {
        let mut store = EvidenceStore::default();
        let id = store.register(sample_record()).unwrap();
        let id = id.0;

        store.revoke(&id).unwrap();
        assert!(store.is_revoked(&id));
        assert_eq!(
            store.revoke(&id).unwrap_err().code,
            "E-EVID-502",
            "double revocation must be refused"
        );
        ...
        let replacement = store
            ...
            .unwrap();
        store.supersede(&id, &replacement.0).unwrap();
        assert_eq!(store.superseded_by(&id), Some(replacement.0.as_str()));
        assert_eq!(
            store
                .supersede(&id, &replacement.0.clone())
                .unwrap_err()
                .code,
            "E-EVID-504",
            "double supersession must be refused"
        );
        assert_eq!(
            store.provenance_of(&replacement.0).unwrap(),
            std::slice::from_ref(&id)
        );
    }

    #[test]
    fn unknown_and_tampered_records_are_refused() {
        let mut store = EvidenceStore::default();
        assert_eq!(
            store.query("fnv1a64:0000000000000000").unwrap_err().code,
            "E-EVID-501"
        );

        // Different content addresses a different slot and is fine.
        let mut different = sample_record();
        different.freshness.issued = "1970-01-01T00:00:00Z".into();
        store.register(different).unwrap();
        assert_eq!(store.query_resolved().len(), 2);

        // A slot whose content no longer matches its address is tamper.
        let id = store.register(sample_record()).unwrap().0;
        let mut tampered_store = store.clone();
        if let Some(record) = tampered_store.records.get_mut(&id) {
            record.freshness.valid_until = "1970-01-01T00:00:00Z".into();
        }
        assert_eq!(
            tampered_store.verify_integrity(&id).unwrap_err().code,
            "E-EVID-503"
        );
