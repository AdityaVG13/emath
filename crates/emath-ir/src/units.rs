...
mod tests {
    ...
    fn meters() -> Unit {
        Unit::si("m".into(), UnitDim::base(1, 0, 0, 0, 0, 0, 0))
    }

    fn seconds() -> Unit {
        Unit::si("s".into(), UnitDim::base(0, 0, 1, 0, 0, 0, 0))
    }

    fn kelvin() -> Unit {
        Unit::si("K".into(), UnitDim::base(0, 0, 0, 0, 1, 0, 0))
    }
    ...
    #[test]
    fn dimensional_check_blocks_invalid_programs() {
        let speed = meters().div(&seconds()).unwrap();
        assert!(check_compatible(&speed, &speed).is_ok());
        let error = check_compatible(&meters(), &kelvin()).unwrap_err();
        assert_eq!(error.code, "E-UNIT-101");
        assert!(error.message.contains("m^1"));
    }

    #[test]
    fn affine_units_refuse_multiplication() {
        let error = celsius().mul(&celsius()).unwrap_err();
        assert_eq!(error.code, "E-UNIT-102");
        assert!(celsius().div(&celsius()).is_err());
        // Same-family addition is allowed by construction (dimension check).
        assert!(check_compatible(&celsius(), &celsius()).is_ok());
    }

    #[test]
    fn identity_is_sealed_and_mutation_sensitive() {
        let meters = meters();
        assert_eq!(meters.identity(), meters.identity());
        let scaled = Unit::new("m".into(), UnitDim::base(1, 0, 0, 0, 0, 0, 0), 100.0, 0.0);
        assert_ne!(meters.identity(), scaled.identity());
        assert_ne!(meters.canonical(), scaled.canonical());
    }

    #[test]
    fn canonical_render_is_stable_golden() {
        let speed = meters().div(&seconds()).unwrap();
        assert_eq!(speed.canonical(), "unit:v1:m/s:m^1*s^-1:1e0:0e0");
    }
}
