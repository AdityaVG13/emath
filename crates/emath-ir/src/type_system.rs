...
mod tests {
    ...
    fn f64() -> TypeExpr {
        TypeExpr::Con(QualifiedName("Float64".into()), Vec::new())
    }

    fn variable(name: &str) -> TypeExpr {
        TypeExpr::Var(TypeVar(name.to_string()))
    }

    #[test]
    fn unify_binds_variables_and_resolves() {
        let mut store = TypeConstraints::default();
        unify(&mut store, &variable("T"), &f64()).unwrap();
        assert_eq!(store.resolve(&variable("T")), f64());
        assert_eq!(store.len(), 1);
    }

    #[test]
    fn unify_same_constructor_is_ok() {
        let mut store = TypeConstraints::default();
        let record = TypeExpr::Con(QualifiedName("Vec".into()), vec![f64(), variable("T")]);
        let record2 = TypeExpr::Con(QualifiedName("Vec".into()), vec![f64(), f64()]);
        unify(&mut store, &record, &record2).unwrap();
    }

    #[test]
    fn unify_conflict_is_traced_error() {
        let mut store = TypeConstraints::default();
        let error = unify(
            &mut store,
            &TypeExpr::Con(QualifiedName("Vec".into()), vec![f64()]),
            &TypeExpr::Con(QualifiedName("Set".into()), vec![f64()]),
        )
        .unwrap_err();
        assert_eq!(error.code, "E-TYPE-312");
        let trace = error.trace(&["infer output", "wire param"]);
        assert!(trace.contains("infer output"));
    }

    #[test]
    fn occurs_check_refuses_recursive_type() {
        let mut store = TypeConstraints::default();
        let ty_equals = TypeExpr::Con(
            QualifiedName("Equals".into()),
            vec![variable("T"), variable("T")],
        );
        let error = unify(&mut store, &variable("T"), &ty_equals).unwrap_err();
        assert_eq!(error.code, "E-TYPE-314");
    }

    #[test]
    fn refinement_discharge_is_classified() {
        assert!(DischargeStatus::Static.is_compile_time());
        assert!(DischargeStatus::Certificate.is_compile_time());
        assert!(!DischargeStatus::RuntimeGuard.is_compile_time());
        assert_eq!(DischargeStatus::RuntimeGuard.name(), "runtime-guard");
    }

    #[test]
    fn schemes_and_canonicals_are_deterministic() {
        let scheme = TypeScheme {
            name: QualifiedName("Point".into()),
            generics: vec!["T".into()],
        ...
        assert!(scheme.canonical().starts_with("scheme:Point<T>"));
        assert_eq!(canonical_of(&f64()), canonical_of(&f64()));
        assert_eq!(
            canonical_of(&TypeExpr::Refined(
                Box::new(f64()),
                "nonneg".into(),
                DischargeStatus::Constructor,
            )),
            "refined(con(Float64,),nonneg,constructor)"
        );
    }
}
