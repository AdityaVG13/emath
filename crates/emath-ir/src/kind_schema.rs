...
mod tests {
    ...
    #[test]
    fn core_function_schema_is_complete() {
        let schema = KindSchema::core_function();
        assert_eq!(schema.name(), "function");
        assert_eq!(
            schema.section("inputs").unwrap().repeat,
            RepeatPolicy::ExactlyOne
        );
        assert_eq!(
            schema.section("compile").unwrap().payload,
            PayloadPolicy::Commands
        );
        assert!(schema.section("compile").unwrap().has_default);
        assert_eq!(
            schema.default_for("compile"),
            Some("rust/library/strict-f64")
        );
        assert!(schema.section("mystery").is_none());
    }

    #[test]
    fn core_policy_adds_state_and_constructors() {
        let schema = KindSchema::core_policy();
        assert!(schema.section("state").is_some());
        assert!(schema.section("constructors").is_some());
        assert_eq!(schema.name(), "policy");
    }

    #[test]
    fn mutating_a_schema_moves_its_identity() {
        let mut schema = KindSchema::core_policy();
        let before = schema.canonical();
        ...
        schema.set_predicate("decl.state.is_empty()");
        assert_ne!(before, schema.canonical());
        assert_eq!(
            schema.section("provenance").unwrap().repeat,
            RepeatPolicy::AtMostOne
        );
    }

    #[test]
    fn payload_policy_allows_only_its_statements() {
        assert!(payload_allows(PayloadPolicy::Fields, "field"));
        assert!(!payload_allows(PayloadPolicy::Fields, "command"));
        assert!(payload_allows(PayloadPolicy::Commands, "command"));
        assert!(payload_allows(PayloadPolicy::Suite, "suite"));
    }

    #[test]
    fn free_function_accessors_match_the_schemas() {
        assert_eq!(
            core_function_schema().canonical(),
            KindSchema::core_function().canonical()
        );
        assert_eq!(
            core_policy_schema().canonical(),
            KindSchema::core_policy().canonical()
        );
    }
}
