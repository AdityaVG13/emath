...
mod tests {
    ...
    #[test]
    fn current_seam_negotiates_clean() {
        let seam = AdapterSeam::current();
        assert!(negotiate(&seam, 0).is_ok());
        assert!(seam.version.compatible_with(0));
        assert!(!seam.version.compatible_with(1));
    }

    #[test]
    fn major_version_drift_is_refused() {
        let seam = AdapterSeam {
            version: ProviderVersion {
                upstream: "dew-2.0.0".into(),
                adapter_protocol: "dew-adapter-1".into(),
            },
            patches: Vec::new(),
        };
        let error = negotiate(&seam, 1).unwrap_err();
        assert_eq!(error.code(), "E-PROV-001");
    }

    #[test]
    fn uncategorized_required_patches_are_refused() {
        let seam = AdapterSeam {
            version: ProviderVersion {
                upstream: "dew-0.1.0".into(),
                adapter_protocol: "dew-adapter-1".into(),
            },
            patches: vec![PatchLedger {
                patch: "unwrap-expression-internals".into(),
                outcome: PatchOutcome::Required,
                motivation: "adapter needs direct access".into(),
            }],
        };
        let error = negotiate(&seam, 0).unwrap_err();
        assert_eq!(error.code(), "E-PROV-002");
    }
}
