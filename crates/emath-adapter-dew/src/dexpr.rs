...
mod tests {
    use super::*;
    use emath_core::Span;
    use emath_ir::{BinaryOp, Literal, UnaryOp};

    fn package_with(nodes: Vec<(ExprNode, Span)>) -> SemanticPackage {
        let mut package = SemanticPackage::new();
        for (node, _) in nodes {
            package.push_expr(node, Span::default());
        }
        package
    }

    #[test]
    fn bit_exact_literal_maps_identity() {
        let package = package_with(vec![(
            ExprNode::Literal(Literal::FloatBits(1u64 << 63)),
            Span::default(),
        )]);
        let mapped = map_expression(&package, ExprId(0)).unwrap();
        assert_eq!(mapped, DewExpr::Float64Bits(1u64 << 63));
    }

    #[test]
    fn strict_arithmetic_maps_one_to_one() {
        let package = package_with(vec![
            (
                ExprNode::Literal(Literal::FloatBits(1u64 << 52)),
                emath_core::Span::default(),
            ),
            (
                ExprNode::Literal(Literal::FloatBits(0x4024_0000_0000_0000)),
                emath_core::Span::default(),
            ),
            (
                ExprNode::Binary {
        ...
        let mapped = map_expression(&package, ExprId(2)).unwrap();
        assert!(matches!(mapped, DewExpr::Mul(..)));
    }

    #[test]
    fn exact_arithmetic_is_refused_before_execution() {
        let package = package_with(vec![
            (
            ...
            (
                ExprNode::Literal(Literal::Integer("2".into())),
                emath_core::Span::default(),
            ),
            (
                ExprNode::Binary {
        ...
        let issue = map_expression(&package, ExprId(2)).unwrap_err();
        assert_eq!(issue.code, "E-PROV-030");
    }

    #[test]
    fn function_naming_table_is_one_to_one() {
        let with_call = package_with(vec![
            (ExprNode::Literal(Literal::FloatBits(0)), Span::default()),
            (
                ExprNode::Call {
                    function: QualifiedName("core::math::abs".into()),
                    arguments: vec![ExprId(0)],
                },
                Span::default(),
            ),
        ]);
        assert!(matches!(
            map_expression(&with_call, ExprId(1)).unwrap(),
            DewExpr::Abs(_)
        ));
        let unknown_package = package_with(vec![
            (ExprNode::Literal(Literal::FloatBits(0)), Span::default()),
            (
                ExprNode::Call {
                    function: QualifiedName("core::math::mystery".into()),
                    arguments: vec![ExprId(0)],
                },
                Span::default(),
            ),
        ...
        assert_eq!(
            map_expression(&unknown_package, ExprId(1))
                .unwrap_err()
                .code,
            "E-PROV-030"
        );
    ...
    #[test]
    fn unary_maps_negate() {
        let package = package_with(vec![
            (
                ExprNode::Literal(Literal::FloatBits(0)),
                emath_core::Span::default(),
            ),
            (
                ExprNode::Unary {
        ...
        assert!(matches!(
            map_expression(&package, ExprId(1)).unwrap(),
            DewExpr::Neg(_)
        ));
    }

    #[test]
    fn linear_algebra_checks_shapes_explicitly() {
        let vector = |len: usize| {
            DewExpr::Matrix(DewMatrix {
                rows: len,
                cols: 1,
                data: vec![DewExpr::Float64Bits(0); len],
        ...
        // dot of equal vectors is admitted.
        let dot = map_linear(LinearOp::Dot, &vector(3), &vector(3)).unwrap();
        assert!(matches!(dot, DewExpr::Linear(..)));
        // matvec m x n . n x 1 is admitted.
        let mv = map_linear(LinearOp::MatVec, &matrix_2x2, &vector(2)).unwrap();
        assert!(matches!(mv, DewExpr::Linear(..)));
    ...
    #[test]
    fn layout_conversion_reorders_deterministically() {
        let value = matrix(
            2,
            2,
            vec![
                DewExpr::Float64Bits(1),
                DewExpr::Float64Bits(2),
                DewExpr::Float64Bits(3),
                DewExpr::Float64Bits(4),
            ],
            Layout::RowMajor,
        );
        let col = convert_layout(&value, Layout::ColMajor);
        let DewExpr::Matrix(col_matrix) = &col else {
            panic!("expected matrix");
        };
        // row-major [1,2,3,4] -> col-major [1,3,2,4]
        assert_eq!(col_matrix.data[0], DewExpr::Float64Bits(1));
        assert_eq!(col_matrix.data[1], DewExpr::Float64Bits(3));
        assert_eq!(col_matrix.data[2], DewExpr::Float64Bits(2));
        assert_eq!(col_matrix.layout, Layout::ColMajor);
        // conversion is idempotent round-trip
        assert_eq!(
            convert_layout(&convert_layout(&value, Layout::ColMajor), Layout::RowMajor),
            value
        );
    }
}
