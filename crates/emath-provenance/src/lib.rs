...
pub fn lineage_closure<A: Adjacency + ?Sized>(
    ...
    let outgoing = adjacency_map(graph);
...
pub fn would_create_cycle<A: Adjacency + ?Sized>(graph: &A, src: NodeId, dst: NodeId) -> bool {
    ...
    while let Some(node) = frontier.pop() {
        for (next, _) in outgoing.get(&node).into_iter().flatten() {
            if *next == src {
                return true;
            }
            if seen.insert(*next) {
...
fn adjacency_map<A: Adjacency + ?Sized>(graph: &A) -> HashMap<NodeId, Vec<(NodeId, EdgeKind)>> {
