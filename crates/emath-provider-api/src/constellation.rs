...
impl MaturityRegistry {
    /// Registers or updates a provider entry.
    pub fn register(&mut self, provider: ConstellationProvider) {
        self.locks
...
pub fn default_constellation() -> MaturityRegistry {
    ...
    for (wave, id, summary, boundary, maturity, owner) in [
        ('A', "wrenfold", "symbolic: conversion subset, simplification/CSE, derivatives/Jacobians", vec!["no proof checking", "no interval arithmetic", "no DAE structural analysis"], MaturityLevel::P0, "backends"),
        ('B', "dew", "expression: rust source/tokens, cranelift JIT, selected accelerator targets", vec!["no tensor derivatives", "no ODE solvers"], MaturityLevel::P0, "execution"),
        ('C', "rumoca", "structural: modelica subset instantiation/flattening, DAE/structural analysis (emath-adapter-rumoca)", vec!["no symbolic integration", "no proof transport"], MaturityLevel::P1, "structural"),
        ('D', "frankenjax", "tensor: dtype/shape representations, tracing, JVP/VJP/Jacobian/Hessian", vec!["no certified numerics", "no FEEC meshes"], MaturityLevel::P0, "tensor"),
        ('D', "frankennumpy", "tensor: ndarray ops, broadcasting, dtype promotion", vec!["no autodiff transforms", "no sparse solvers"], MaturityLevel::P0, "tensor"),
        ('E', "frankenscipy", "numerical: root solving, quadrature, ODE/BVP, linear/sparse solvers, optimization, special functions", vec!["no symbolic manipulation", "no proof checking"], MaturityLevel::P0, "numerics"),
        ('F', "frankensim", "simulation: typed operators, FEEC/mesh/physics models, interval/certified numerics", vec!["no general autodiff", "no remote execution"], MaturityLevel::P0, "simulation"),
        ('G', "frankenlean", "proof: statement/proof transport, kernel checking, theorem/certificate evidence", vec!["no numerical execution", "no tensor ops"], MaturityLevel::P0, "proof"),
        ('H', "frankenengine", "runtime evidence: deterministic replay, signed evidence, guarded execution/promotion", vec!["no math semantics", "no codegen"], MaturityLevel::P0, "runtime"),
    ] {
        registry.register(ConstellationProvider {
            id: id.to_string(),
            wave,
...
mod tests {
    ...
    #[test]
    fn census_covers_all_waves_with_boundaries() {
        let registry = default_constellation();
        let waves: Vec<char> = registry.providers().iter().map(|p| p.wave).collect();
        for wave in ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'] {
            assert!(waves.contains(&wave), "wave {wave} missing");
        }
        for provider in registry.providers() {
            assert!(
                !provider.no_claim_boundary.is_empty(),
                "{} claims everything",
                provider.id
            );
        }
    ...
    #[test]
    fn rumoca_starts_at_p1_and_locks() {
        let registry = default_constellation();
        assert_eq!(registry.maturity_of("rumoca"), Some(MaturityLevel::P1));
        assert_eq!(
            registry.lock_of("rumoca").unwrap().last_compatible(),
            Some(&"lock-P1".to_string())
        );
    }

    #[test]
    fn promotion_requires_criteria_and_no_skipped_rungs() {
        let mut registry = default_constellation();
        let proofs = MaturityLevel::P0
            .next_criteria()
            .iter()
            .map(|c| (*c).to_string())
            .collect::<Vec<_>>();
        // P0 -> P1 requires the P0 criteria only.
        assert_eq!(
            registry
                .promote("wrenfold", MaturityLevel::P1, &proofs)
                .unwrap(),
            MaturityLevel::P1
        );
        assert_eq!(registry.maturity_of("wrenfold"), Some(MaturityLevel::P1));
        // Jumping P1 -> P3 without the P1 criteria is refused.
        let error = registry
            .promote("dew", MaturityLevel::P3, &proofs)
            .unwrap_err();
        assert_eq!(error.code, "E-PROV-523");
        // Unknown provider.
        assert_eq!(
            registry
                .promote("nope", MaturityLevel::P1, &proofs)
                .unwrap_err()
                .code,
            "E-PROV-521"
        );
    ...
    #[test]
    fn rollback_preserves_last_compatible_version() {
        let mut registry = default_constellation();
        // Fresh lock: only the P0 entry.
        assert_eq!(
            registry.lock_of("dew").unwrap().last_compatible(),
            Some(&"lock-P0".to_string())
        );
        // Promote to P1; both locks are on record.
        let proofs: Vec<String> = MaturityLevel::P0
            .next_criteria()
            .iter()
            .chain(MaturityLevel::P1.next_criteria())
            .map(|c| (*c).to_string())
            .collect();
        registry.promote("dew", MaturityLevel::P1, &proofs).unwrap();
        assert_eq!(
            registry.lock_of("dew").unwrap().last_compatible(),
            Some(&"lock-P1".to_string())
        );
        // Breaking P1 rolls back to the last known compatible P0 lock.
        registry.rollback("dew", "lock-P1").unwrap();
        assert_eq!(
            registry.lock_of("dew").unwrap().last_compatible(),
            Some(&"lock-P0".to_string())
        );
        // Breaking the only remaining version leaves no compatible lock.
        registry.rollback("dew", "lock-P0").unwrap();
        assert_eq!(registry.lock_of("dew").unwrap().last_compatible(), None);
    }

    #[test]
    fn disable_rolls_back_but_keeps_census() {
        let mut registry = default_constellation();
        registry.disable("frankenjax").unwrap();
        assert_eq!(
            compose_chain(&["frankenjax".to_string()], &registry),
            CompositionOutcome::Parametric {
                missing: "frankenjax".into()
            }
        );
        registry.enable("frankenjax").unwrap();
        assert!(matches!(
            compose_chain(&["frankenjax".to_string()], &registry),
            CompositionOutcome::Artifact(_)
        ));
        assert_eq!(registry.providers().len(), 9);
    }

    #[test]
    fn composition_is_deterministic_and_type_free() {
        let registry = default_constellation();
        let chain = vec![
            "wrenfold".to_string(),
            "dew".to_string(),
        ...
        let first = compose_chain(&chain, &registry);
        assert_eq!(compose_chain(&chain, &registry), first);
        // Registry contains only ids/versions, no provider types.
        assert_eq!(chain.first().unwrap(), "wrenfold");
    }

    #[test]
    fn missing_provider_lifts_to_parametric() {
        let registry = default_constellation();
        let outcome = compose_chain(
            &[
                "frankenlean".to_string(),
                "frankensim".to_string(),
        ...
        assert_eq!(
            outcome,
            CompositionOutcome::Parametric {
                missing: "missing.vendor".into()
            }
        );
    }

    #[test]
    fn census_rows_are_stable_golden() {
        let registry = default_constellation();
        let census: String = registry
            .providers()
            .iter()
            .find(|provider| provider.id == "rumoca")
            .map(|provider| provider.census())
            .unwrap_or_default();
        assert_eq!(
            census,
            "rumoca|C|P1|enabled|structural: modelica subset instantiation/flattening, DAE/structural analysis (emath-adapter-rumoca)|no symbolic integration;no proof transport|structural"
        );
    }
}
