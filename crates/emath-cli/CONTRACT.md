...
## Conformance tests
- No `tests/` directory in the crate. Workspace suites in `tests/emath-cli` (`tests/catalog.rs`, `tests/intent_completion_solve.rs`, `tests/v9_06_2rdq_6_unit.rs`, `tests/meaning_lock.rs`, `tests/lib.rs`); the keep-gate bench harness is `tests/emath-cli/benches/keep_gate.rs` (not a test). Library-unit coverage comes from `emath-sema`, `emath-build`, and `emath-checker`.
