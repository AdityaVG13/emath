...
mod tests {
    ...
    fn pilot() -> CachePilot {
        ...
        let selector = Selector::new(gate, PromotionOutcome::Promote, 0).unwrap();
        ...
        .unwrap();
        CachePilot::new(selector, monitor)
    }

    #[test]
    fn pilot_serves_and_tracks_application_metrics() {
        let mut pilot = pilot();
        assert!(!pilot.serve("k", 100, 50, true).cached);
        let second = pilot.serve("k", 100, 50, true);
        assert!(second.cached);
        let _ = pilot.serve("k2", 100, 50, true);
        let _ = pilot.serve("k", 100, 50, true);
        assert_eq!(pilot.served(), 4);
        assert!((pilot.hit_rate() - 0.5).abs() < 1e-12);
        assert_eq!(pilot.telemetry().served_candidate, 4);
    }

    #[test]
    fn pilot_survives_candidate_failure_with_fallback() {
        let mut pilot = pilot();
        let result = pilot.serve("k", 100, 50, false);
        assert!(result.correct);
        assert_eq!(result.latency_ns, 100);
        assert_eq!(pilot.telemetry().fallbacks, 1);
        // Host keeps serving.
        assert!(pilot.serve("k2", 100, 50, true).correct);
    }

    #[test]
    fn latency_drift_is_detected_and_reported() {
        let mut pilot = pilot();
        let _ = pilot.serve("k", 100, 50, true);
        assert!(pilot
            .observe_latency(200, 100)
            .iter()
            .any(|alert| alert.code() == "E-HOST-010"));
        assert_eq!(pilot.alerts().len(), 1);
    }

    #[test]
    fn pilot_produces_a_sealed_receipt() {
        let pilot = pilot();
        let receipt = pilot.into_receipt(
            emath_core::ContentId("exp".into()),
            r#"{"schema":"lab:v1"}"#.into(),
            crate::stats::StatisticalProtocol {
                warmup_repetitions: 0,
                repetitions: 1,
                min_repetitions: 1,
                paired: true,
                seed: 0,
                outlier: crate::OutlierPolicy::KeepAll,
                retain_raw: true,
            ...
            None,
            false,
            crate::promotion::PromotionDecision {
                outcome: PromotionOutcome::Retain,
                reason: crate::promotion::PromotionReason::ManualHold,
            },
            "pilot --serve".into(),
            "macos-arm64".into(),
            vec![],
        );
        assert!(receipt.receipt_id.0.starts_with("fnv1a64:"));
    }
}
