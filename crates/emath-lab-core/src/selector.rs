...
mod tests {
    ...
    #[test]
    fn dispatch_tracks_routes_deterministically() {
        let selector = Selector::new(open_gate(), PromotionOutcome::Canary, 3).unwrap();
        let mut selector = selector;
        assert_eq!(selector.dispatch(0), Route::Canary);
        assert_eq!(selector.dispatch(1), Route::Baseline);
        assert_eq!(selector.dispatch(2), Route::Baseline);
        assert_eq!(selector.dispatch(3), Route::Canary);
        assert_eq!(selector.telemetry().served_canary, 2);
        assert_eq!(selector.telemetry().served_baseline, 2);
    }

    #[test]
    fn closed_gate_cannot_route_candidate() {
        let closed = QualityGate::evaluate(vec![GateCheck::fail(
            "correctness",
            GateCheckKind::Correctness,
            "E-HOST-005",
            "out of tolerance",
        )]);
        let error = Selector::new(closed, PromotionOutcome::Promote, 0).unwrap_err();
        assert_eq!(error.code, "E-HOST-005");
    }

    #[test]
    fn deoptimization_steps_back_and_counts() {
        let mut selector = Selector::new(open_gate(), PromotionOutcome::Promote, 0).unwrap();
        assert!(selector.deoptimize(&PromotionReason::ManualHold));
        assert_eq!(selector.outcome(), PromotionOutcome::Retain);
        assert_eq!(selector.telemetry().deoptimizations, 1);
        assert_eq!(selector.dispatch(0), Route::Baseline);
        assert!(!selector.deoptimize(&PromotionReason::ManualHold));
    }

    #[test]
    fn failure_fallback_serves_baseline() {
        let mut selector = Selector::new(open_gate(), PromotionOutcome::Promote, 0).unwrap();
        assert_eq!(selector.on_failure(), Route::Baseline);
        assert_eq!(selector.telemetry().fallbacks, 1);
    }
}
