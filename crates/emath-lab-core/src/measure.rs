...
mod tests {
    ...
    #[test]
    fn summary_is_deterministic_and_pins_percentiles() {
        let measurement = Measurement {
            metric_id: "latency".into(),
            kind: MeasurementKind::LatencyNs,
            unit: "ns".into(),
            samples: vec![100, 90, 80, 70, 60, 50, 40, 30, 20, 10],
        };
        let summary = measurement.summarize().unwrap();
        assert_eq!(summary.count, 10);
        assert_eq!(summary.min, 10);
        assert_eq!(summary.max, 100);
        assert!((summary.median - 55.0).abs() < 1e-9);
        assert!((summary.p90 - 91.0).abs() < 1e-9);
        assert!((summary.p99 - 99.1).abs() < 1e-9);
        assert!((summary.mean - 55.0).abs() < 1e-9);
    }

    #[test]
    fn empty_measurement_is_insufficient_evidence() {
        let measurement = Measurement::single("none", MeasurementKind::LatencyNs, "ns", 0);
        let empty = Measurement {
            samples: vec![],
            ..measurement
        };
        let error = empty.summarize().unwrap_err();
        assert_eq!(error.code, "E-HOST-006");
    }

    #[test]
    fn throughput_and_energy_are_explicit_models() {
        assert!((Measurement::throughput_ops_per_sec(10, 1_000_000) - 10_000.0).abs() < 1e-9);
        let model = EnergyCostModel::default();
        let joules = model.energy_joules(1_000_000, 0.0);
        assert!(joules > 0.0);
        assert!((model.device_cost_usd(3.6e6) - 0.15).abs() < 1e-12);
    }

    #[test]
    fn memory_ratio_comes_from_peaks() {
        let mut report = HarnessReport {
            baseline: vec![Measurement::single(
                "peak",
                MeasurementKind::PeakMemoryBytes,
                "bytes",
                100,
            )],
            candidate: vec![Measurement::single(
                "peak",
                MeasurementKind::PeakMemoryBytes,
                "bytes",
                120,
        ...
        report.sort();
        let ratio = report.peak_memory_ratio().unwrap();
