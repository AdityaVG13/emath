...
mod tests {
    ...
    fn honest() -> RunFacts {
        RunFacts {
            baseline_input_fingerprint: Some("inputs-v1".into()),
            candidate_input_fingerprint: Some("inputs-v1".into()),
            baseline_operations: 1_000,
            candidate_operations: 800,
            baseline_warmup_repetitions: 2,
            candidate_warmup_repetitions: 2,
            poisoned_inputs_injected: 10,
            correctness_failures_observed: 3,
            calibration_joined_decision_corpus: false,
            baseline_profile: "release".into(),
            candidate_profile: "release".into(),
            baseline_toolchain: "1.74.0".into(),
            candidate_toolchain: "1.74.0".into(),
        }
    }

    #[test]
    fn honest_experiment_passes_every_detector() {
        let facts = honest();
        assert!(run_all(&facts).iter().all(|check| check.passes));
        assert!(require_comparable(&facts).is_ok());
    }

    #[test]
    fn cheating_and_changed_inputs_are_refused() {
        let mut cheating = honest();
        cheating.candidate_operations = 0;
        assert!(require_comparable(&cheating)
            .unwrap_err()
            .message
            .contains("benchmark-cheating"));

        let mut changed = honest();
        changed.candidate_input_fingerprint = Some("inputs-v2".into());
        let error = require_comparable(&changed).unwrap_err();
        assert_eq!(error.code, "E-HOST-008");
        assert!(error.message.contains("changed-inputs"));
    }

    #[test]
    fn asymmetric_warmup_and_missing_failures_are_refused() {
        let mut warmup = honest();
        warmup.candidate_warmup_repetitions = 4;
        assert!(require_comparable(&warmup)
            .unwrap_err()
            .message
            .contains("asymmetric-warmup"));

        let mut failures = honest();
        failures.correctness_failures_observed = 0;
        assert!(require_comparable(&failures)
            .unwrap_err()
            .message
            .contains("missing-failures"));
    }

    #[test]
    fn poisoned_calibration_and_build_mismatch_are_refused() {
        let mut calibration = honest();
        calibration.calibration_joined_decision_corpus = true;
        assert!(require_comparable(&calibration)
            .unwrap_err()
            .message
            .contains("poisoned-calibration"));

        let mut builds = honest();
        builds.candidate_profile = "debug".into();
        assert!(require_comparable(&builds)
            .unwrap_err()
            .message
            .contains("non-comparable-builds"));
    }
}
