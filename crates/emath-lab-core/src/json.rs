...
mod tests {
    ...
    fn write_sorts_object_keys() {
        let value = JsonValue::Object(vec![
            ...
            ("a".into(), JsonValue::String("x".into())),
        ]);
        assert_eq!(write(&value), r#"{"a":"x","b":2}"#);
    }

    #[test]
    fn parse_round_trips_nested_documents() {
        let text = r#"{"b":[1,2.5,-3e2,true,null,"a\nb\u00e9"],"c":{"d":-0.5}}"#;
        let parsed = parse(text).unwrap();
        assert_eq!(
            write(&parsed),
            r#"{"b":[1,2.5,-300,true,null,"a\nbé"],"c":{"d":-0.5}}"#
        );
    }

    #[test]
    fn parse_accepts_whitespace_and_rejects_junk() {
        let parsed = parse("  { \"x\" : [ 1 , 2 ] }  ").unwrap();
        assert_eq!(write(&parsed), r#"{"x":[1,2]}"#);
    ...
    #[test]
    fn string_escapes_are_deterministic() {
        assert_eq!(
            write(&JsonValue::String("a\"b\\c\nd\u{08}".into())),
            r#""a\"b\\c\nd\b""#
        );
    }
