...
mod tests {
    ...
    #[test]
    fn macro_expand_rejects_non_literal() {
        let error = macro_expand("x + 1").unwrap_err();
        assert!(error.message.contains("string literal"));
    }

    #[test]
    fn macro_expand_accepts_source_literal() {
        let expansion = macro_expand("\"y = x * x\"").unwrap();
        assert_eq!(expansion.source, "y = x * x");
        assert_eq!(expansion.identity, expansion.identity);
    }

    #[test]
    fn from_literals_reconstructs_expansion() {
        let expansion = MacroExpansion::from_literals("y = x", "fnv1a64:0000000000000000");
        assert_eq!(expansion.source, "y = x");
        assert_eq!(expansion.identity, "fnv1a64:0000000000000000");
    }

    #[test]
    fn build_from_model_runs_the_artifact_path() {
        let dir = std::env::temp_dir().join(format!("emath-builder-model-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let model = BuilderModel::custom("SquareModel")
            .kind(KindRef::Function)
            .input("x", TypeKind::Float64)
            .output("y", TypeKind::Float64)
            .define(
        ...
        let report = build_from_model(model, "square.emath", &dir).unwrap();
        assert!(report.artifact_dir.join("src/lib.rs").is_file());
        let lib = std::fs::read_to_string(report.artifact_dir.join("src/lib.rs")).unwrap();
        assert!(lib.contains("pub struct SquareModel"));
        let _ = std::fs::remove_dir_all(&dir);
    }

    #[test]
    fn build_from_source_shares_the_compiler_path() {
        let dir = std::env::temp_dir().join(format!("emath-builder-source-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&dir);
        let source = r"emath custom <SrcModel> as function:
    inputs:
        x: Float64
    outputs:
        y: Float64
...
";
        let report = build_from_source("src.emath", source, &dir).unwrap();
        assert!(report
            .artifact_dir
