...
mod tests {
    ...
    #[test]
    fn validates_and_renders_alien_term() {
        let (signature, term) = alien_term();
    ...
    fn rejects_wrong_arity() {
        ...
        let term = Term::Apply {
            operator: SymbolId("⋈".into()),
            arguments: vec![Term::Variable(VariableId("a".into()))],
        };
        assert!(matches!(
            signature.validate(&term),
            Err(TermError::ArityMismatch { .. })
        ));
    }

    #[test]
    fn canonical_round_trips_alien_term() {
        let (_, term) = alien_term();
        let text = term.canonical();
        assert_eq!(Term::parse_canonical(&text).unwrap(), term);
    }

    #[test]
    fn canonical_round_trips_escaped_names() {
        let term = Term::Apply {
            operator: SymbolId("a\\(b,c)\\\\d".into()),
            arguments: vec![Term::Constant(SymbolId("x,y".into()))],
        };
        let text = term.canonical();
        assert_eq!(Term::parse_canonical(&text).unwrap(), term);
    }
    ...
    #[test]
    fn canonical_rejects_trailing_and_malformed() {
        assert_eq!(
            Term::parse_canonical("var(a) extra"),
            Err(CanonicalError::Trailing {
                text: "var(a) extra".into()
            })
        );
        assert!(matches!(
            Term::parse_canonical("apply(⋈,var(a)"),
