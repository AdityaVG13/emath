...
### Sum binder authority

`std.binder.sum` defines `sum x in domain if guard: body` as a finite left
fold in canonical domain order through `std.capability.math.add`. The guard and
body bind `x` only inside their expressions. An empty domain returns the
additive identity. Exact Int sums use checked arithmetic; overflow is
`exactness-loss`, never wrapping or float widening.

Nonfinite domains require an admitted method/continuation and otherwise produce
a typed open-domain diagnosis. Reordering an approximate or order-sensitive
carrier requires explicit equivalence evidence. The authored capsule is
[`../spec/binders/core/sum.emath`](../spec/binders/core/sum.emath); the runnable
finite example is [`../examples/intro/sum-first-n.emath`](../examples/intro/sum-first-n.emath).
Sum adds no Stage-0/parser/sema/backend name branch: the generic binder and
capability dispatch read its capsule/image records.
...
The Float64 builtins `abs`/`min`/`max` are piecewise selections; `sqrt`
carries the `SqrtNonNegative` domain obligation on the caller (a
negative operand yields the IEEE NaN value on the Float64 carrier, not
a parse refusal); `atan2` is the IEEE correctly-rounded two-argument
arctangent; `sign` is mathematical sgn with exactly `0` at zero. All
six admit end-to-end in the strict-f64 subset (emath-s9w1m + fpl60);
the runnable probe is
`language/examples/intro/clamp-distance-builtins.emath`.

`mod` is prefix only: `mod(a, b)`. Infix `a mod b` does not parse
today; a definition using it diagnoses with `E-SYN-101`.

Special-function calls have matching `<name>_error_bound` accessors.
Poles and exits from the declared real carrier are labeled runtime
`fault` values (`E-SPECIAL-POLE`, `E-SPECIAL-DOMAIN`); `elliptic_pi`
currently diagnoses `E-SPECIAL-NOT-IMPLEMENTED` (no world computes it
yet).
...
A single binder may carry a filter after the domain:

```emath
sum i in 0..n if i < k: matrix[i, i]
```

Multiple binder variables compose in one fold, comma-separated. The
fold desugars to nested single-binder folds: the leftmost binder is the
outermost loop and the rightmost is the innermost (the standard
iterated-integral convention for `integral`); an optional filter binds
to the innermost binder, and may reference the outer binders:

```emath
sum i in 0..n, j in 0..m: weight[i, j]
sum i in 0..n, j in 0..n if j == i: 1          # diagonal count
sum i in 0..n, j in 0..i: 1                    # inner domain may reference outer binders
```
...
The same quadrature is also exposed as a program-carrier goal cell through the `std.capability.calculus.simpson-integral` capsule and its `checked-simpson-integral` kernel: the body is an authored reference program, the supplied vector is the body's complete `Float64` environment, and explicit value arguments carry both bounds, the step count, and the loop-variable index. Steps must be positive, even, and within the reference op's `u16` range; the loop variable is bound in the named environment slot at every node; a body result that is not `Float64` refuses with `E-TYPE-012`.
...
The admitted recurrence is a finite linear combination of earlier terms with contiguous base cases. A same-index or forward reference diagnoses with `E-SEQ-TERMINATION`; malformed or missing base cases diagnose with `E-SEQ-RECURRENCE`.
...
ASCII `~=` is equivalent to `≈`. A missing tolerance is `E-APPROX-TOL`. At runtime the check is `abs(left-right) <= atol + rtol*abs(right)`; failure faults the run.
...
`jacobian(body) wrt v1, v2, ...` (Track A3, emath-9bj1) computes the Jacobian as a value: a list body `[f1, f2]` differentiates component-wise (one row per component, one column per `wrt` variable), a scalar body yields a single row (`Matrix[1, n]`). Ordering is source order: row `i` is component `i` of the list in written order and column `j` is the `j`-th `wrt` variable in written order; no sorting, deduplication, or reordering ever applies. A component that is not a scalar number (vector, matrix, or a nested `jacobian`) diagnoses with `E-TYPE-012`; it is never silently flattened. The form is parse-time sugar for a matrix literal of `derivative` cells, so it uses the same forward-mode engine as `derivative`; no second engine and no domain ops. Unsupported shapes and non-numeric (nondifferentiable) bodies are typed diagnoses (`E-TYPE-012`); a `wrt` name that is not an input diagnoses with the derivative form's input-scope code (`E-TYPE-010`). `hessian` is not admitted yet; it diagnoses as an unknown keyword.
...
The supplied runtime values are initial guesses. Non-convergence, singular derivatives or Hessians, and wrong curvature are labeled `fault` outcomes, never fabricated answers. Optimization strategy names are explicit; methods never gain authority merely by being declared.

The scalar case is also exposed as a program-carrier goal cell through the `std.capability.calculus.scalar-solve` capsule and its `checked-newton-scalar-solve` kernel. The seed is the named environment slot; a root is returned only when `|residual| < tolerance` after at least one Newton iteration; zero or negative budgets refuse before any iteration, a vanished derivative refuses with the existing typed text, and non-finite iterates or an exhausted budget refuse rather than fabricate a root. The kernel's derivative is the documented forward difference (epsilon `1e-8`); the dual-number derivative and the deterministic bracket-scan fallback remain the authority of the `solve` expression form itself.

`solve` runs Newton's method with a deterministic robustness fallback (Track A3, emath-9bj1): when the derivative vanishes or the residual/step becomes non-finite, the solver scans a fixed geometric grid (alternating ± steps around the seed, ×8 growth, 48 levels) for a sign-changing bracket and bisects it with a fixed 120-iteration budget. A root is reported only when `|residual| < tolerance`; no bracket; or a divergent bisection; is a typed diagnosis, never a hang and never an invented root. All scan constants are fixed, so the fallback is bit-deterministic across runs and seeds.
...
The admitted carrier is a real two-level system. Basis labels outside `0` and `1` diagnose. Unmounted glyphs are `E-SYN-157`.

### Nabla notation

The nabla pack maps declared gradient, divergence, curl, and Laplacian glyphs to the corresponding spatial builtins. Dimensional mismatches, such as three-dimensional curl on a two-dimensional field, diagnose.
...
Edge syntax is valid only inside a graph literal. Malformed or dangling edges diagnose at parse time.
...
A hole may contain only a name or dotted path. Calls, indexing, and arbitrary expressions are diagnosed. The fixed format suffix is `.Nf`; doubled braces escape literal braces. String templates evaluate to NFC-normalized `Text`. The intentionally small text surface is `nfc`, `text_length`, equality, report construction, and pure rendering; arbitrary text-processing operations remain diagnosed.
...
Select the structural method with `--method backward-euler` or `--method velocity-verlet`. Velocity Verlet requires two scalar states satisfying `q' = v` and `v' = a(q)`; dissipative or velocity-dependent acceleration diagnoses with `E-ODE-002`. Invalid time steps, non-finite coefficients, and nonlinear solve failure are typed diagnoses.

## Events and transitions

Two cooperating, generic surfaces (ch7) let a
...
surface: duplicate names diagnose with `E-NAME-022`, anything that is not an
event declaration diagnoses with `E-SYN-101`.
...
same name (an action-less suite is diagnosed: the payload arm must contain
...
against `algebraic:` unknowns are diagnosed (`E-TRANS-005`): the Newton
...
there is no capture source and admission diagnoses with `E-TRANS-006` (naming the
parameter and the event). At a firing whose capture is otherwise missing
at runtime, the diagnosis is `E-TRANS-007`.
...
admission replay and diagnosis-text reproducibility). No RNG; the harness is
...
diagnosed or faulted during evaluation), `E-EVENT-009` (event action value
non-finite; NaN/±Inf never poisons a slot, the run exits with the fault).
...
(transition action value non-finite; never poison, the run exits with the fault).

All event/transition faults are typed diagnoses, never silent drops. In
...
the algebraic unknown) diagnoses through the causalized-Newton projection
with the raw `E-DAE-INIT` / `Regularize` typed diagnosis text; the run
returns the diagnosis and **never a partial trajectory**.
...
Domain capabilities use ordinary calls rather than new expression grammars. Examples include polynomial evaluation and multiplication, linear solves and decompositions, shortest paths and spectral graph operations, transfer-function evaluation, and seeded probability sampling. Their exact names, shapes, numeric policies, and diagnosis codes are documented in `standard-library-constitution.md` and `language/stdlib/cells/`.
