...
Normative status of the geometry pack. Geometry is
...
## 3. Named fences and their diagnoses

- `cross` over a `Vector[2]` (or any out-of-bounds component read):
  diagnosed typed at execution (index fault), never a fabricated third
  component. Today the admission surface does not shape-check the
  index; the honest contract is a labeled fault at execution and the
  admission-time shape diagnosis is part of the call-seam work below.
- `normalize(0-vector)`: diagnoses (non-finite at execution), never
...
- `Option[Point3D]` ray results: diagnosed `E-TYPE-010` until the
  generic `Option<T>` surface lands. The
...
- Named builtins `cross`, `normalize`, `distance` do not exist;
  ...
  (§4). An unknown name still diagnoses `E-TYPE-003`; nothing became a
  builtin.
...
`distance` is `norm(a − b)`). Unknown names still diagnose typed, and
the strict Rust codegen backend does not lower capability
applications yet (`emath run` diagnoses typed; the reference VM / interp
...
**User-defined functions are callable too:** a call whose
...
runtime callee frame; recursion diagnoses typed (`E-TYPE-013`, inline
depth cap 32), arity/type mismatches diagnose `E-TYPE-012`, and a
...
values, arity diagnosis, reparameterization invariance, determinism,
mutation-killed).
