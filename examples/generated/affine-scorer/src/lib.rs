...
mod emath_rt {
    ...
    pub fn tensor_index_checked(
        ...
        let expected =
            shape_product(shape).ok_or(IndexError::Arithmetic("tensor size overflow"))?;
    ...
    pub fn einsum_checked(
        ...
        for (spec, (shape, _)) in input_specs.iter().zip(operands.iter()) {
            ...
            let mut local: std::collections::HashMap<char, usize> =
                std::collections::HashMap::new();
        ...
        for (out_pos, out_coord) in out_coords.iter().enumerate() {
            let mut idx_map: std::collections::HashMap<char, usize> =
                std::collections::HashMap::new();
    ...
    // ── Stiff + symplectic ODE nucleus (thin slice) ────────────────────
    //
    ...
    // ── Spectral Poisson, 1D Dirichlet (thin slice) ────────────────────
    //
    ...
    // ── Probability: seeded sampling + densities (thin slice) ─────────
    //
    ...
    pub fn new(scale: f64, bias: f64) -> Result<Self, ConfigError> {
        {
            let __ok0 = !{
                matches!(
                    emath_rt::cmp_i64_f64((0i64), (scale)),
            ...
            let __ok1 = !{ (scale).is_finite() };
            if __ok1 {
                {
                    return Err(ConfigError::FailedPrecondition);
                }
            }
            let __ok2 = !{ (bias).is_finite() };
            if __ok2 {
                {
                    return Err(ConfigError::FailedPrecondition);
                }
