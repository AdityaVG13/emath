#!/usr/bin/env python3
"""Ambiguity scan for emath grammar deltas (ELP gate).
...
This is the mechanical first-pass battery for the governance-bug
class; it is not a full ambiguity proof. The ELP procedure
(`elps/README.md`) pairs it with the boundary corpus and human review.

Usage:
    ambiguity_scan.py [--delta DELTA.diff] [grammar.ebnf ...]
...
def scan_terms(rhs: str, line: int) -> list:
    ...
    while i < n:
        ...
        if ch.isspace():
            ...
            continue
        if ch == ",":
            # ISO EBNF: a bare comma is the sequence separator between
            # terms, never a terminal — a real comma glyph is the quoted
            # literal `","` (caught by the quote branch below). Skipping
            # separators keeps first-sets and glyph roles free of phantom
            # commas: a nullable prefix must fall through to the next
            # TERM, not to the separator between two terms.
            i += 1
            continue
        if ch == "?":
...
def diff_targets(diff: str, path: str) -> bool:
    ...
    and applied everywhere (the original battery behavior). Diffs with a
...
def _payload_firsts(payload: str, line: int, productions: dict, seen: set) -> tuple:
    """(firsts, any_branch_nullable) over a bracketed payload's branches.

    A bare `|` inside `(...)`, `[...]`, `{...}` is the choice operator of
    the dialect, never a glyph: the payload's first set is the UNION over
    its top-level branches. A linear scan would stop at the `|` as if it
    were an opaque terminal and truncate the group to its first branch
    (e.g. `("unit" | "dimension")` reporting only `unit`)."""
    firsts = set()
    any_nullable = False
    for branch in split_top_level(payload):
        branch = branch.strip()
        if not branch:
            any_nullable = True
            continue
        f, n = seq_first(scan_terms(branch, line), productions, seen)
        firsts |= f
        any_nullable = any_nullable or n
    return (firsts, any_nullable)
...
def term_first(term: Term, productions: dict, seen: set) -> tuple:
    ...
    if term.kind == "ref":
        if term.value in seen:
            # First-position recursion (a production whose first-set
            # derivation re-enters a production already on this chain):
            # the back-edge contributes no terminals and must not
            # fabricate nullability — claiming nullable here lets the
            # scan fall through the back-edge into the rest of the
            # graph, blowing first sets up to "everything reachable".
            return (set(), False)
        firsts = first_set(term.value, productions, seen | {term.value})
        nullable = _production_nullable(term.value, productions, seen)
        return (firsts, nullable)
    if term.kind == "empty":
        return (set(), True)
    if term.kind == "optional":
        firsts, _ = _payload_firsts(term.value, term.line, productions, seen)
        return (firsts, True)
    if term.kind == "repeat":
        firsts, _ = _payload_firsts(term.value, term.line, productions, seen)
        return (firsts, True)  # zero repetitions always possible
    if term.kind == "group":
        return _payload_firsts(term.value, term.line, productions, seen)
    if term.kind == "opt":
...
def _term_nullable(term: Term, productions: dict, seen: frozenset) -> bool:
    """True when a term can derive the empty string."""
    if term.kind in ("empty", "optional", "repeat", "opt"):
        return True
    if term.kind == "ref":
        return _production_nullable(term.value, productions, seen)
    if term.kind == "group":
        return _payload_nullable(term.value, term.line, productions, seen)
    return False  # literal, special


def _payload_nullable(payload: str, line: int, productions: dict, seen: frozenset) -> bool:
    """A bracketed payload is nullable when any of its `|` branches is."""
    for branch in split_top_level(payload):
        branch = branch.strip()
        if not branch:
            return True
        if all(_term_nullable(t, productions, seen) for t in scan_terms(branch, line)):
            return True
    return False


def _production_nullable(name: str, productions: dict, seen: frozenset) -> bool:
    """A production is nullable when any alternative is entirely nullable."""
    if name not in productions:
        return False
    if name in seen:
        return False  # recursion never fabricates nullability
    return any(
        _alt_nullable(alt, productions, seen | {name})
        for alt in productions[name].alternatives
    )


def _alt_nullable(alt: list, productions: dict, seen: frozenset = frozenset()) -> bool:
    """An alternative is nullable iff EVERY term in it is nullable.

    The earlier `any` semantics declared `identifier { "::" identifier }`
    nullable because it CONTAINS a repeat, letting first sets fall
    through the mandatory leading identifier to phantom terminals
    (a record's `:`, an assignment's `.`/`[`, a quantity's space)."""
    return all(_term_nullable(t, productions, seen) for t in alt)
...
def scan(productions: dict) -> list:
    ...
    for name, production in productions.items():
        nullables = [_alt_nullable(alt, productions) for alt in production.alternatives]
...
def main(argv: list) -> int:
    ...
    try:
        ...
        print(f"ambiguity-scan: cannot parse grammar: {exc}", file=sys.stderr)
        return 2
    ...
    if args.emit_baseline:
        ...
        with open(args.emit_baseline, "w", encoding="utf-8") as fh:
            ...
            fh.write("\n")
        print(
            f"ambiguity-scan: baseline written to {args.emit_baseline} "
    ...
    for severity, _, message in pinned:
        print(f"ambiguity-scan: info (baseline): {message}")
    for severity, _, message in new_errors:
        print(f"ambiguity-scan: error: {message}")
    for severity, _, message in warnings:
        print(f"ambiguity-scan: warning: {message}")
    print(
        f"ambiguity-scan: {len(merged)} productions; {len(new_errors)} NEW errors, "
