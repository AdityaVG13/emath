#!/usr/bin/env python3
"""Confusable-glyph scan for emath grammar deltas (ELP gate).
...
Usage:
    confusable_scan.py [--delta DELTA.diff] [grammar.ebnf ...]
...
def main(argv: list) -> int:
    ...
    for message in warnings:
        print(f"confusable-scan: warning: {message}")
    for message in errors:
        print(f"confusable-scan: error: {message}")
    print(f"confusable-scan: {len(existing)} glyphs; {len(errors)} errors, {len(warnings)} warnings")
