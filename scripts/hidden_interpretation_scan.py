#!/usr/bin/env python3
"""Hidden-interpretation scan for emath grammar deltas (ELP gate).

One glyph, many meanings: the fourth mechanical sub-test. A quoted
...
Silently picking one meaning at parse time is the governance-bug
class this battery prevents.

The registry (`tests/language-gates/fixtures/
hidden-interpretation-baseline.json`) records every currently
...
- `parser-context` — the parser/operator table or lexer position
  ...
  class; `precedence_battery.py` prints the operator-table resolution).
...
Reuses the EBNF parser from `ambiguity_scan.py` so every battery
measures the same grammar dialect.

Usage:
    hidden_interpretation_scan.py [--delta DELTA.diff]
...
from ambiguity_scan import apply_diff, diff_targets, parse_grammar
...
def _walk_terms(terms: list, production: str, roles: dict) -> None:
    ...
    three)."""
    from ambiguity_scan import scan_terms, split_top_level
...
def main(argv: list) -> int:
    ...
    try:
        ...
        print(f"hidden-interpretation-scan: cannot parse grammar: {exc}", file=sys.stderr)
        return 2
    ...
    if args.emit_baseline:
        ...
        print(
            f"hidden-interpretation-scan: baseline written to {args.emit_baseline} "
    ...
    if args.baseline:
        try:
            ...
            print(f"hidden-interpretation-scan: error: {exc}", file=sys.stderr)
            return 1
    ...
    for severity, _, message in infos:
        print(f"hidden-interpretation-scan: info: {message}")
    for severity, _, message in errors:
        print(f"hidden-interpretation-scan: error: {message}")
    for severity, _, message in warnings:
        print(f"hidden-interpretation-scan: warning: {message}")
    multi = sum(1 for names in roles.values() if len(names) >= 2)
    print(
        f"hidden-interpretation-scan: {len(roles)} glyphs; {multi} multi-role; "
