...
//! emath capstone demos: `cargo xtask demo holes-synthesis`,
...
//! runs binder expansion through the semantic VM and emits a
//! deterministic receipt; the math-layout demo runs the LaTeX and
...
fn main() {
    ...
    let code = if args.first().map(String::as_str) == Some("demo") {
        match args.get(1).map(String::as_str) {
            ...
            other => {
                eprintln!(
                    "unknown demo {other:?}; usage: cargo xtask demo <holes-synthesis|scoped-binders|math-layout|interpretation-portfolio|finite-analogues|finite-worlds|agent-meaning|world-morphisms|joint-tuning|source-first-worlds|meaning-store|portable-emlib|easy-math|cache-policy>"
                );
                2
            }
        }
...
/// Binder production path: builds all six binder kinds across the four
...
/// Math-layout production path: parse a mixed LaTeX document, extract the
