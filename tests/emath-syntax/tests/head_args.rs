...
";
    ...
    probe.case("cache_policy_example_parses_as_one_declaration", |p| {
    let f0 = p.failures().len();
...
";
    ...
    if p.failures().len() != f0 { return; }
    p.demand("2", (names) == (vec!["AdaptiveCachePolicy".to_string()]), format!("expected {:?}, got {:?}", (vec!["AdaptiveCachePolicy".to_string()]), (names)));
    if p.failures().len() != f0 { return; }
    p.demand("3",sections.contains(&"goals".to_string()), format!(
