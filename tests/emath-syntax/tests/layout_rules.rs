//! Layout diagnostics + bracket idiom.
//!
...
fn layout_f2() {
    boot();
    let mut probe = Probe::new("Layout diagnostics + bracket idiom. The layout rule: NEWLINE fires at statement boundaries and is suppressed inside `()[]{}`. The bracket idiom");
