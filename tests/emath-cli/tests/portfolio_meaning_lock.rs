...
fn probe() {
    ...
    p.case("codec-utf8", |p| { let mut lock = MeaningLock::with_cap(5); lock.upsert(LockKey { declaration_id: 0x1111_1111_1111_1111, hole_id: "\u{22c8}".to_string() }, LockEntry { source: "glyphs-\u{22c8}.emath".into(), source_hash: 0x2222_2222_2222_2222, world_fingerprint: 0x3333_3333_3333_3333, portfolio_receipt_id: 0x4444_4444_4444_4444, selection_method: SelectionMethod::CliSet, selected_at: 1_700_000_000 }); let encoded = lock.encode(); let round = MeaningLock::parse(&encoded).expect("parse"); p.eq("bytes", round.encode().as_bytes().to_vec(), encoded.as_bytes().to_vec()); p.eq("resolve", round.resolve(0x1111_1111_1111_1111, "\u{22c8}", "glyphs-\u{22c8}.emath").expect("resolve").expect("entry").world_fingerprint, 0x3333_3333_3333_3333); });
