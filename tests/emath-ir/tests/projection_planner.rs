...
#[test]
fn intent() {
    let mut p = Probe::new("Projection planner and per-class closure");
    p.case("pure_cell_full_closure_is_stable", |p| {
    ...
    p.demand(format!("full closure must be stable, got: {refusals:?}"), refusals.is_empty(), format!("full closure must be stable, got: {refusals:?}"));
    ...
    p.eq("pure_cell_full_closure_is_stable#2", plan.iter().find(|(k, _)| *k == ProjectionKind::Identity), Some(&(ProjectionKind::Identity, ProjectionStatus::Generated)));
    p.eq("pure_cell_full_closure_is_stable#3", plan.iter().find(|(k, _)| *k == ProjectionKind::Schema), Some(&(ProjectionKind::Schema, ProjectionStatus::Generated)));
    p.eq("pure_cell_full_closure_is_stable#4", plan.iter().find(|(k, _)| *k == ProjectionKind::Semantics), Some(&(ProjectionKind::Semantics, ProjectionStatus::Provided)));
    p.demand("closed plan carries no refusals", !plan.iter().any(|(_, s)| *s == ProjectionStatus::Refused), "closed plan carries no refusals");

    });
    p.case("pure_cell_without_reference_blocks_stable", |p| {
    ...
    p.eq(format!("both pure-only gaps surface: {refusals:?}"), refusals.len(), 2);
    p.demand("pure_cell_without_reference_blocks_stable#2", refusals.iter().any(|r| r.code() == "E-CELL-007"), "pure_cell_without_reference_blocks_stable#2: refusals.iter().any(|r| r.code() == \"E-CELL-007\")");
    p.demand("refusal names the cell", refusals
            .iter()
            .any(|r| r.cell_name() == "std.tensor.softmax"), "refusal names the cell");
    ...
    p.eq("pure_cell_without_reference_blocks_stable#4", plan.iter().find(|(k, _)| *k == ProjectionKind::Reference), Some(&(ProjectionKind::Reference, ProjectionStatus::Refused)));

    });
    p.case("drifted_docs_rejected", |p| {
    ...
    let refusals = missing_required(&schema, &supplied);
    p.demand(format!("stale docs binding drifts from cell id: {refusals:?}"), refusals.iter().any(|r| r.code() == "E-CELL-008"), format!("stale docs binding drifts from cell id: {refusals:?}"));
    ...
    p.demand("docs must declare the cell id they document", missing_required(&schema, &unbound)
            .iter()
            .any(|r| r.code() == "E-CELL-008"), "docs must declare the cell id they document");

    });
    p.case("provider_matrix_not_applicable_and_boundary", |p| {
    ...
    p.eq("pure: 7 universal + reference + compilation", required_projections(CellClass::Pure).len(), 9);
    p.eq("provider: 7 universal only", required_projections(CellClass::Provider).len(), 7);
    p.eq("theory: 7 universal only", required_projections(CellClass::Theory).len(), 7);
    ...
    p.demand("provider closure is stable without reference/compilation", missing_required(&schema, &supplied).is_empty(), "provider closure is stable without reference/compilation");

    let plan = plan_cell_closure(&schema, &supplied);
    p.eq("provider_matrix_not_applicable_and_boundary#5", plan.iter().find(|(k, _)| *k == ProjectionKind::Reference), Some(&(ProjectionKind::Reference, ProjectionStatus::NotApplicable)));
    p.eq("provider_matrix_not_applicable_and_boundary#6", plan.iter().find(|(k, _)| *k == ProjectionKind::Compilation), Some(&(ProjectionKind::Compilation, ProjectionStatus::NotApplicable)));
    p.eq("provider_matrix_not_applicable_and_boundary#7", plan.iter().find(|(k, _)| *k == ProjectionKind::Semantics), Some(&(ProjectionKind::Semantics, ProjectionStatus::Provider)));
    ...
    p.demand("provider_matrix_not_applicable_and_boundary#9", refusals[0].code() == "E-CELL-007", format!("expected {:?}, got {:?}", "E-CELL-007", refusals[0].code()));
