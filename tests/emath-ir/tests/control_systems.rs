...
//! `language/spec/capabilities/numerics/dynamics-control-pde.emath`, whose
//! authored reference programs install with the checked-in Language Image.
...
fn install_language() {
    ...
    install_language_distribution(&distribution).expect("control reference cells install");
}
...
fn refused_code(fault: &EvalFault) -> String {
    match fault {
        EvalFault::CapabilityRefused { code, .. } => code.clone(),
        EvalFault::CarrierRefused { detail, .. } => detail.clone(),
        _ => panic!("expected a typed capability refusal, got {fault:?}"),
    }
}
...
#[test]
fn intent() {
    let mut p = Probe::new("(thin B43 control surface): transfer");
    p.case("transfer_function_evaluates_known_rational", |p| {
...
// wrong Horner or a wrong division).
    ...
    p.eq("H(-3) = 3/2", f64_of(&at_minus_3), 1.5);

    });
    p.case("transfer_function_zero_denominator_refuses", |p| {
...
// value does not exist — never Inf-by-silent-convention.
    ...
    p.demand("transfer_function_zero_denominator_refuses#1", refused_code(&fault) == "E-CONTROL-002", format!("expected {:?}, got {:?}", "E-CONTROL-002", refused_code(&fault)));
    for zero_den in [Value::Vector(vec![]), Value::Vector(vec![0.0, 0.0])] {
        ...
        p.demand("transfer_function_zero_denominator_refuses#2", refused_code(&fault) == "E-CONTROL-002", format!("expected {:?}, got {:?}", "E-CONTROL-002", refused_code(&fault)));
    }

    });
    p.case("transfer_function_non_finite_input_refuses", |p| {
...
// corrupted ratio.
    ...
    p.demand("transfer_function_non_finite_input_refuses#1", refused_code(&fault) == "E-CONTROL-001", format!("expected {:?}, got {:?}", "E-CONTROL-001", refused_code(&fault)));
    let num = Value::Vector(vec![6.0, 1.0]);
    let fault = eval(
        vec![cell(
            TRANSFER_EVAL,
    ...
    p.demand("transfer_function_non_finite_input_refuses#2", refused_code(&fault) == "E-CONTROL-001", format!("expected {:?}, got {:?}", "E-CONTROL-001", refused_code(&fault)));

    });
    p.case("dc_gain_matches_transfer_function", |p| {
...
// stability gate breaks the bit parity or the refused unstable twin.
    ...
    p.eq("c·(−A)⁻¹·b for the companion pair", f64_of(&dc), 0.5);
    let h0 = eval(
        vec![cell(
            TRANSFER_EVAL,
            vec![EmirValue(0), EmirValue(1), EmirValue(2)],
    ...
    .expect("transfer evaluates at s = 0");
    p.eq("bit-exact cross-form parity", f64_of(&h0).to_bits(), f64_of(&dc).to_bits());
    ...
    p.demand("dc_gain_matches_transfer_function#3", refused_code(&fault) == "E-CONTROL-003", format!("expected {:?}, got {:?}", "E-CONTROL-003", refused_code(&fault)));

    });
    p.case("dc_gain_shape_refusals", |p| {
...
// law, never silently truncated or broadcast.
    ...
    p.demand("dc_gain_shape_refusals#1", refused_code(&fault) == "E-CONTROL-004", format!("expected {:?}, got {:?}", "E-CONTROL-004", refused_code(&fault)));
    ...
    p.demand("dc_gain_shape_refusals#2", refused_code(&fault) == "E-CONTROL-004", format!("expected {:?}, got {:?}", "E-CONTROL-004", refused_code(&fault)));

    });
    p.case("poles_and_stability_laws_hold", |p| {
...
// `E-CONTROL-001`. ASCENDING carriers, same as B28.
    ...
    p.demand("(s+1)(s+2) is strictly stable", bool_of(&stable(Value::Vector(vec![2.0, 3.0, 1.0]), "quadratic")), "(s+1)(s+2) is strictly stable");
    p.demand("(s+1)³ is strictly stable", bool_of(&stable(Value::Vector(vec![1.0, 3.0, 3.0, 1.0]), "cubic")), "(s+1)³ is strictly stable");
    let unstable = stable(Value::Vector(vec![2.0, 1.0, 0.5, 1.0]), "unstable cubic");
    p.demand("s³ + ½s² + s + 2 has two RHP roots", !bool_of(&unstable), "s³ + ½s² + s + 2 has two RHP roots");
    let fault = eval(
        vec![cell(POLES_STABLE, vec![EmirValue(0)])],
        &[Value::Vector(vec![1.0, 0.0, 1.0])],
    )
    .expect_err("s² + 1 is marginal (zero first-column entry)");
    p.demand("poles_and_stability_laws_hold#5", refused_code(&fault) == "E-CONTROL-005", format!("expected {:?}, got {:?}", "E-CONTROL-005", refused_code(&fault)));
    for degenerate in [Value::Vector(vec![]), Value::Vector(vec![0.0])] {
        ...
        p.demand("poles_and_stability_laws_hold#6", refused_code(&fault) == "E-CONTROL-002", format!("expected {:?}, got {:?}", "E-CONTROL-002", refused_code(&fault)));
    }
    let fault = eval(
        vec![cell(POLES_STABLE, vec![EmirValue(0)])],
    ...
    p.demand("poles_and_stability_laws_hold#7", refused_code(&fault) == "E-CONTROL-001", format!("expected {:?}, got {:?}", "E-CONTROL-001", refused_code(&fault)));

    });
    p.case("control_cells_preserve_parity_and_guards", |p| {
...
// `E-CELL-006` all-finite cell guard surfaces one layer lower now).
    ...
    p.eq("H(0) = 6/2 through the seam", f64_of(&cell_value), 3.0);
    ...
    p.eq("c·(−A)⁻¹·b through the seam", f64_of(&cell_dc), 0.5);
    let cell_stable = eval(vec![cell(POLES_STABLE, vec![EmirValue(0)])], &[den.clone()])
        .expect("stability cell computes");
    p.demand("(s+1)(s+2) is strictly stable", bool_of(&cell_stable), "(s+1)(s+2) is strictly stable");
    let fault = eval(
        vec![cell(
            TRANSFER_EVAL,
            vec![EmirValue(0), EmirValue(1), EmirValue(2)],
    ...
    p.demand("control_cells_preserve_parity_and_guards#4", refused_code(&fault) == "E-CONTROL-001", format!("expected {:?}, got {:?}", "E-CONTROL-001", refused_code(&fault)));

    });
    p.case("control_shape_refusals", |p| {
...
// compiler; the carrier law itself is capsule data now.
    ...
    p.demand(format!("the transfer capsule refuses the wrong carrier typed: {fault:?}"), refused_code(&fault).starts_with("E-CONTROL"), format!("the transfer capsule refuses the wrong carrier typed: {fault:?}"));
    ...
    p.demand("control_shape_refusals#2", refused_code(&fault) == "E-CONTROL-004", format!("expected {:?}, got {:?}", "E-CONTROL-004", refused_code(&fault)));
