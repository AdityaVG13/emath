//! emath-xx0x.5 (thin nucleus slice): probability distributions +
//! seeded sampling at the EMIR seam.
//!
//! The bead's law, thinned to the compute layer:
...
//!   Uniform(a, b), Bernoulli(p) — each as `ProbSample` (seeded
//!   draws) and `ProbDensity` (exact density/PMF evaluation).
...
use emath_exec_ir::{EmirOp, EmirProgram, EmirValue, EvalBudget, ProbKind};
...
fn sample(kind: EmirOp, params: &[f64], seed: f64, draws: f64) -> Result<Vec<f64>, EvalFault> {
    let value = eval(
        vec![kind],
        &[
...
fn seed_reproducibility_bit_exact() {
    // THE bead law: fixed seed ⟹ deterministic draws across runs and
...
fn declared_stream_paths_split_and_replay() {
    let sample_path = |path: &str| {
        let value = eval(
            vec![EmirOp::ProbSample {
                kind: ProbKind::Normal,
                params: EmirValue(0),
                seed: EmirValue(1),
                draws: EmirValue(2),
                stream: Some(EmirValue(3)),
            }],
            &[
...
fn uniform_range_and_moments() {
    ...
    .expect("uniform sample computes");
...
fn densities_closed_forms() {
    ...
    let density = |kind: ProbKind, params: &[f64], x: f64| -> Result<f64, EvalFault> {
        let value = eval(
            vec![EmirOp::ProbDensity {
                kind,
                params: EmirValue(0),
                x: EmirValue(1),
            }],
    ...
    let normal_pdf_0 = density(ProbKind::Normal, &[0.0, 1.0], 0.0).expect("pdf computes");
    assert!(
        (normal_pdf_0 - 1.0 / two_pi_sqrt).abs() < 1e-12,
        "normal pdf(0) = 1/√(2π), got {normal_pdf_0}"
    );
    let uniform_mid = density(ProbKind::Uniform, &[2.0, 5.0], 3.5).expect("pdf computes");
    assert!(
        (uniform_mid - 1.0 / 3.0).abs() < 1e-12,
        "uniform density = 1/(b−a), got {uniform_mid}"
    );
    let uniform_out =
        density(ProbKind::Uniform, &[2.0, 5.0], 9.0).expect("pdf computes");
    assert_eq!(uniform_out, 0.0, "uniform density outside [a, b] is 0");
    let pmf_one = density(ProbKind::Bernoulli, &[0.3], 1.0).expect("pmf computes");
    let pmf_zero = density(ProbKind::Bernoulli, &[0.3], 0.0).expect("pmf computes");
...
fn invalid_parameters_refuse_typed() {
    ...
    let cases: Vec<(ProbKind, Vec<f64>, f64, &str)> = vec![
        (ProbKind::Normal, vec![0.0, 0.0], 1.0, "sigma zero"),
        (ProbKind::Normal, vec![0.0, -1.0], 1.0, "sigma negative"),
        (ProbKind::Uniform, vec![5.0, 2.0], 1.0, "a > b"),
        (ProbKind::Bernoulli, vec![1.5], 1.0, "p over one"),
        (ProbKind::Bernoulli, vec![-0.1], 1.0, "p under zero"),
        (ProbKind::Normal, vec![0.0, 1.0], 2.5, "fractional draws"),
    ];
    for (kind, params, draws, label) in cases {
        let error = sample(
            EmirOp::ProbSample {
                kind,
                params: EmirValue(0),
...
fn cell_registry_and_shape_law() {
    // The .emath surface: std.prob.normal_sample is registry DATA
    // (cohort 28), compiles through the call seam, and evaluates the
    // SAME draws as the bare op. A scalar params slot refuses at
    // COMPILE (ShapeMismatch) — the closed vocabulary's shape law.
    let registry = std_cell_registry();
    for name in [
        "std.prob.normal_sample",
        "std.prob.uniform_sample",
        "std.prob.bernoulli_sample",
        "std.prob.normal_density",
        "std.prob.uniform_density",
        "std.prob.bernoulli_pmf",
    ] {
        assert!(
    ...
    let term = Term::Apply {
        operator: SymbolId("normal_sample".into()),
        arguments: vec![
    ...
    signature
        .insert(SymbolId("normal_sample".into()), 3)
        .expect("normal_sample signature is conflict-free");
    let params = vec![
        ("params".to_string(), ParamShape::Vector),
        ("seed".to_string(), ParamShape::Scalar),
        ("draws".to_string(), ParamShape::Scalar),
    ];
    compile_reference(&term, &signature, &params, Vec::new(), "std.prob.normal_sample")
        .expect("vector params compile");

    // Shape law: a scalar params slot refuses at COMPILE.
    let scalar_params = vec![
        ("params".to_string(), ParamShape::Scalar),
        ("seed".to_string(), ParamShape::Scalar),
        ("draws".to_string(), ParamShape::Scalar),
    ];
    ...
    .expect_err("scalar params refuse at compile");
    let compile_error = format!("{error:?}");
    assert!(
        compile_error.contains("ShapeMismatch"),
        "scalar params must ShapeMismatch at compile, got {compile_error}"
    );
    let _ = TermCompileError::ShapeMismatch {
        symbol: "normal_sample".to_string(),
