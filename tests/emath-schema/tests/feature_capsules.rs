...
#[test]
fn probe() {
    let mut probe = Probe::new("feature capsules: every check in one probe");
    probe.case("exact_add_capsule_carries_executable_reference_body", |probe| {

        let text = exact_add_text();
        let (capsule, issues) = parse_feature_capsule(&text);
        probe.demand("\"exact add capsule refuses: {issues:?}\"", issues.is_empty(), format!("exact add capsule refuses: {issues:?}"));
        ...
        probe.eq("params_text", &(params_text), &("lhs,rhs"));
        probe.eq("signature_text", &(signature_text), &("add=2"));
        ...
        probe.eq("term.canonical()", &(term.canonical()), &(*term_text));
        ...
        probe.eq("capsule.semantic_hash", &(capsule.semantic_hash), &(capsule_semantic_hash(&text).unwrap()));

        // An executable body stays an authored reference.
        probe.eq("capsule.slots.get(\"reference\")", &(capsule.slots.get("reference")), &(Some(&CapsuleSlot::Value("authored".to_string()))));
    });
    probe.case("reference_body_participates_in_semantic_hash", |probe| {

        let text = exact_add_text();
        let swapped = text.replace("var(lhs),var(rhs)", "var(rhs),var(lhs)");
        probe.ne("capsule_semantic_hash(&text).unwrap()", &(capsule_semantic_hash(&text).unwrap()), &(capsule_semantic_hash(&swapped).unwrap()));
        let (_, issues) = parse_feature_capsule(&swapped);
        probe.demand("\"mutated body must refuse its stale declared hash: {issues:?}\"", issues.iter().any(|issue| issue.code == "E-CAPSULE-022"), format!("mutated body must refuse its stale declared hash: {issues:?}"));
    });
    probe.case("executable_reference_refusals_are_typed_and_stable", |probe| {

        let text = exact_add_text();
        let expect_code = |probe: &mut Probe, mutated: &str, code: &str| {
            let (_, issues) = parse_feature_capsule(mutated);
            probe.demand(code, issues.iter().any(|issue| issue.code == code), format!("missing {code}: {issues:?}"));
        };
        ...
        expect_code(probe, &conflicting, "E-CAPSULE-024");
        // Free variable outside the declared parameter list.
        let free = text.replace("reference_params: \"lhs,rhs\"", "reference_params: \"rhs\"");
        expect_code(probe, &free, "E-CAPSULE-025");
        // Executable body with a non-authored reference mode.
        let mode = text.replace("reference: \"authored\"", "reference: \"provider\"");
        expect_code(probe, &mode, "E-CAPSULE-026");
    });
    probe.case("all_twenty_class_rules_cover_fifty_six_canonical_seed_capsules", |probe| {

        probe.eq("CLASS_RULES.len()", &(CLASS_RULES.len()), &(20));
        let mut canonical = Vec::new();
        for ordinal in 0..56 {
            let class = FeatureClass::ALL[ordinal % FeatureClass::ALL.len()];
            let text = seed(class, ordinal);
            let (capsule, issues) = parse_feature_capsule(&text);
            probe.demand("\"{class:?} seed {ordinal}: {issues:?}\"", issues.is_empty(), format!("{class:?} seed {ordinal}: {issues:?}"));
            let capsule = capsule.unwrap();
            probe.eq("capsule.feature_id.class()", &(capsule.feature_id.class()), &(class.as_str()));
            let first = capsule.canonical_bytes();
            let second = parse_feature_capsule(&text).0.unwrap().canonical_bytes();
            probe.eq("first", &(first), &(second));
            canonical.push(first);
        }
        probe.eq("canonical.len()", &(canonical.len()), &(56));
    });
    probe.case("capsule_boundaries_refuse_untyped_or_falsely_live_data", |probe| {
        ...
        for (needle, replacement, code) in [
            ...
            probe.demand("\"missing {code}: {issues:?}\"", issues.iter().any(|issue| issue.code == code), format!("missing {code}: {issues:?}"));
        }
        ...
        probe.demand("issues.iter().any(|issue| issue.code == \"E-CAPSULE-016\")", issues.iter().any(|issue| issue.code == "E-CAPSULE-016"), "issues.iter().any(|issue| issue.code == \"E-CAPSULE-016\")");
        ...
        probe.demand("issues.iter().any(|issue| issue.code == \"E-CAPSULE-017\")", issues.iter().any(|issue| issue.code == "E-CAPSULE-017"), "issues.iter().any(|issue| issue.code == \"E-CAPSULE-017\")");
    });
    probe.case("maturity_transitions_are_explicit_and_bounded", |probe| {
        ...
        probe.eq("validate_maturity_transition(Maturity::Cataloged, Maturity::Stable) .unwrap_err...", &(validate_maturity_transition(Maturity::Cataloged, Maturity::Stable)
                .unwrap_err()
                .code), &("E-CAPSULE-020"));
    });
    probe.case("semantic_hash_excludes_presentation_and_rejects_domain_substitution", |probe| {

        let first = seed(FeatureClass::Capability, 61);
        let changed = first.replace("presentation: seed", "presentation: infix plus");
        probe.eq("capsule_semantic_hash(&first).unwrap()", &(capsule_semantic_hash(&first).unwrap()), &(capsule_semantic_hash(&changed).unwrap()));
        let (capsule, issues) = parse_feature_capsule(&first);
        probe.demand("issues.is_empty()", issues.is_empty(), "issues.is_empty()");
        probe.eq("capsule.unwrap().semantic_hash", &(capsule.unwrap().semantic_hash), &(capsule_semantic_hash(&first).unwrap()));
        probe.demand("SemanticHash::from_str(&format!( \"distribution-{}\", capsule_semantic_hash(&firs...", SemanticHash::from_str(&format!(
                "distribution-{}",
                capsule_semantic_hash(&first).unwrap()
            ))
            .is_err(), "SemanticHash::from_str(&format!( \"distribution-{}\", capsule_semantic_hash(&firs...");
        ...
        probe.ne("capsule_semantic_hash(&two_edges).unwrap()", &(capsule_semantic_hash(&two_edges).unwrap()), &(capsule_semantic_hash(&changed_second).unwrap()));
        let duplicate_operational = format!("{two_edges}path: first\npath: second\n");
        probe.eq("capsule_semantic_hash(&duplicate_operational) .unwrap_err() .code", &(capsule_semantic_hash(&duplicate_operational)
                .unwrap_err()
                .code), &("E-CAPSULE-021"));
    });
    probe.finish();
